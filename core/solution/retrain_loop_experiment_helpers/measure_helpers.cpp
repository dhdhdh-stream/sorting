...
void RetrainLoopExperiment::measure_activate(
		Problem* problem,
...
void RetrainLoopExperiment::measure_backprop(double target_val) {
	...
	if (this->state_iter >= solution->curr_num_datapoints) {
		...
		if (combined_improvement_t_score > 2.326) {	// >99%
			...
			this->state = RETRAIN_LOOP_EXPERIMENT_STATE_VERIFY_EXISTING;
			this->state_iter = 0;
		} else {
