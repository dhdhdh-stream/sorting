...
/**
 * - don't bother trying to train new state
 *   - will only be state for the current path anyways
 */
void RetrainLoopExperiment::measure_existing_activate(
		Problem* problem,
...
void RetrainLoopExperiment::measure_existing_backprop(
		double target_val,
		RunHelper& run_helper) {
	...
	if ((int)this->o_target_val_histories.size() >= solution->curr_num_datapoints) {
		...
		this->state = RETRAIN_LOOP_EXPERIMENT_STATE_TRAIN_HALT;
		this->state_iter = 0;
	}
}
