...
void Fold::input_step_explore_off_path_activate(
	...
	if (!this->is_inner_scope[this->finished_steps.size()]) {
		problem.perform_action(this->actions[this->finished_steps.size()]);
		...
		vector<double> scope_input(solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->num_inputs);
		for (int i_index = 0; i_index < solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->num_inputs; i_index++) {
		...
		ScopeHistory* scope_history = new ScopeHistory(solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]);
		solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->explore_off_path_activate(
			problem,
			scope_input,
			scope_output,
			predicted_score,
...
void Fold::input_step_explore_off_path_backprop(
	...
	if (history->exit_index < (int)this->finished_steps.size()) {
		...
		// will be empty if !this->is_inner_scope
		...
		if (!this->is_inner_scope[this->finished_steps.size()]) {
			...
			vector<double> scope_output_errors;
			double scope_scale_factor_error = 0.0;
			solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->explore_off_path_backprop(
				scope_input_errors,
				scope_output_errors,
				predicted_score,
				target_val,
			...
			for (int s_index = 0; s_index < solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->num_inputs; s_index++) {
			...
			this->curr_inner_input_network->backprop_small_errors_with_no_weight_change(
				scope_output_errors,
...
void Fold::input_step_existing_flat_activate(
		Problem& problem,
		double starting_score,
		vector<double>& local_s_input_vals,
		vector<double>& local_state_vals,
	...
	if (!this->is_inner_scope[this->finished_steps.size()]) {
		...
		vector<double> scope_output;
		ScopeHistory* scope_history = new ScopeHistory(solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]);
		solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->existing_flat_activate(
			problem,
			scope_input,
			scope_output,
			predicted_score,
...
void Fold::input_step_existing_flat_backprop(
	...
	if (history->exit_index < (int)this->finished_steps.size()) {
		...
		if (!this->is_inner_scope[this->finished_steps.size()]) {
			...
			vector<double> scope_output_errors;
			double scope_scale_factor_error = 0.0;
			solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->existing_flat_backprop(
				scope_input_errors,
				scope_output_errors,
				predicted_score,
				predicted_score_error,
...
void Fold::input_step_update_activate(
		Problem& problem,
		double starting_score,
		vector<double>& local_s_input_vals,
		vector<double>& local_state_vals,
	...
	if (!this->is_inner_scope[this->finished_steps.size()]) {
		...
		vector<double> scope_output;
		ScopeHistory* scope_history = new ScopeHistory(solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]);
		solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->update_activate(
			problem,
			scope_input,
			scope_output,
			predicted_score,
	...
	if (this->input_networks.size() > 0) {
		...
		input_errors = vector<double>(this->input_sizes.back(), 0.0);
	}
	...
	history->curr_score_network_history = score_network_history;
	history->score_update = this->curr_score_network->output->acti_vals[0];
	...
	if (this->input_networks.size() > 0) {
		...
		int initial_size = (int)this->test_score_network->s_input_input->errors.size() - this->input_sizes.back();
		for (int s_index = 0; s_index < this->input_sizes.back(); s_index++) {
			input_errors[s_index] = this->test_score_network->s_input_input->errors[initial_size+s_index];
	...
	if (this->curr_compress_num_layers > 0) {
		if (this->curr_compress_new_size > 0) {
			...
			for (int s_index = 0; s_index < this->curr_compress_new_size; s_index++) {
				compress_errors[s_index] = this->curr_compress_network->output->acti_vals[s_index]
					- this->test_compress_network->output->acti_vals[s_index];
			...
			if (this->input_networks.size() > 0) {
				...
				int initial_size = (int)this->test_compress_network->s_input_input->errors.size() - this->input_sizes.back();
				for (int s_index = 0; s_index < this->input_sizes.back(); s_index++) {
					input_errors[s_index] += this->test_compress_network->s_input_input->errors[initial_size+s_index];
...
void Fold::input_step_update_backprop(
	...
	if (history->exit_index < (int)this->finished_steps.size()) {
		...
		if (!this->is_inner_scope[this->finished_steps.size()]) {
			...
			double scope_scale_factor_error = 0.0;
			solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->update_backprop(
				predicted_score,
				target_val,
				final_misguess,
				scale_factor,
...
void Fold::input_step_existing_update_activate(
		Problem& problem,
		double starting_score,
		vector<double>& local_s_input_vals,
		vector<double>& local_state_vals,
	...
	if (!this->is_inner_scope[this->finished_steps.size()]) {
		...
		vector<double> scope_output;
		ScopeHistory* scope_history = new ScopeHistory(solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]);
		solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->existing_update_activate(
			problem,
			scope_input,
			scope_output,
			predicted_score,
...
void Fold::input_step_existing_update_backprop(
	...
	if (history->exit_index < (int)this->finished_steps.size()) {
		...
		if (!this->is_inner_scope[this->finished_steps.size()]) {
			...
			double scope_scale_factor_error = 0.0;
			solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->existing_update_backprop(
				predicted_score,
				predicted_score_error,
				scale_factor,
				scope_scale_factor_error,
...
void Fold::input_step_update_increment(FoldHistory* history,
	...
	if (history->exit_index < (int)this->finished_steps.size()) {
		...
		if (!this->is_inner_scope[this->finished_steps.size()]) {
			...
			solution->scope_dictionary[this->existing_scope_ids[this->finished_steps.size()]]->update_increment(
				history->scope_histories[this->finished_steps.size()],
				folds_to_delete);
