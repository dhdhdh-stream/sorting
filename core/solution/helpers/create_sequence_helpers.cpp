...
Sequence* create_sequence(Problem& problem,
	...
	for (int c_index = 0; c_index < (int)temp_context.size(); c_index++) {
		for (int n_index = 0; n_index < (int)temp_context[c_index].added_recursion_protection_flags.size(); n_index++) {
			run_helper.recursion_protection_flags.erase(temp_context[c_index].added_recursion_protection_flags[n_index]);
		}
	}
