		...
		if ((int)this->i_target_val_histories.size() >= solution->curr_num_datapoints) {
			if (this->sub_state_iter == 0) {
				...
				for (int d_index = 0; d_index < solution->curr_num_datapoints; d_index++) {
					...
					for (int i_index = 0; i_index < (int)this->branch_network_input_indexes.size(); i_index++) {
						network_inputs[d_index][i_index] = vector<double>((int)this->branch_network_input_indexes[i_index].size());
						for (int s_index = 0; s_index < (int)this->branch_network_input_indexes[i_index].size(); s_index++) {
							network_inputs[d_index][i_index][s_index] = input_vals[this->branch_network_input_indexes[i_index][s_index]];
						}
					}
				}
				for (int i_index = 0; i_index < (int)this->input_scope_contexts.size(); i_index++) {
				...
				for (int d_index = 0; d_index < solution->curr_num_datapoints; d_index++) {
					outputs(d_index) = this->i_target_val_histories[d_index] - this->branch_average_score;
				}
				...
				if (this->branch_network == NULL) {
					...
					this->branch_average_misguess = sum_misguesses / solution->curr_num_datapoints;

					double sum_misguess_variances = 0.0;
					for (int d_index = 0; d_index < solution->curr_num_datapoints; d_index++) {
						double curr_misguess = diffs(d_index) * diffs(d_index);
						sum_misguess_variances += (curr_misguess - this->branch_average_misguess) * (curr_misguess - this->branch_average_misguess);
					}
					this->branch_misguess_variance = sum_misguess_variances / solution->curr_num_datapoints;
				} else {
					...
					this->branch_average_misguess = final_average_misguess;
				...
				for (int d_index = 0; d_index < solution->curr_num_datapoints; d_index++) {
					...
					for (int i_index = 0; i_index < (int)this->input_scope_contexts.size(); i_index++) {
						linear_impact += input_vals[i_index] * this->branch_linear_weights[i_index];
					}
					network_target_vals[d_index] = this->i_target_val_histories[d_index] - this->branch_average_score - linear_impact;
				...
				if (this->branch_network == NULL) {
					...
					test_network = new Network(this->branch_network);
				...
				if (t_score > 2.326) {
					...
					this->branch_network_input_indexes.push_back(new_input_indexes);

					delete this->branch_network;
					this->branch_network = test_network;

					this->branch_average_misguess = average_misguess;
					this->branch_misguess_variance = misguess_variance;
				} else {
					delete test_network;
				}
			}
			...
			this->sub_state_iter++;
			if (this->sub_state_iter >= RETRAIN_BRANCH_EXPERIMENT_TRAIN_ITERS) {
