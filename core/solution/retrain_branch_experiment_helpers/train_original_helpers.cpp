...
void RetrainBranchExperiment::train_original_backprop(
	...
	if (history->has_target) {
		...
		if ((int)this->i_target_val_histories.size() >= solution->curr_num_datapoints) {
			...
			this->original_input_state_weights = vector<map<int, double>>(this->branch_node->branch_scope_context.size());
			this->original_local_state_weights = vector<map<int, double>>(this->branch_node->branch_scope_context.size());
			this->original_temp_state_weights = vector<map<State*, double>>(this->branch_node->branch_scope_context.size());
			...
			if (stride_size > 0) {
				...
				for (int i_index = 0; i_index < solution->curr_num_datapoints; i_index++) {
					outputs(i_index) = this->i_target_val_histories[i_index] - this->original_average_score;
				}
				...
				Eigen::VectorXd weights = inputs.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(outputs);
				{
					...
					for (int c_index = 0; c_index < (int)this->branch_node->branch_scope_context.size(); c_index++) {
						for (map<int, vector<double>>::iterator it = p_input_state_vals[c_index].begin();
								it != p_input_state_vals[c_index].end(); it++) {
							this->original_input_state_weights[c_index][it->first] = weights(s_index);
							s_index++;
						}
					}
					...
					for (int c_index = 0; c_index < (int)this->branch_node->branch_scope_context.size(); c_index++) {
						for (map<int, vector<double>>::iterator it = p_local_state_vals[c_index].begin();
								it != p_local_state_vals[c_index].end(); it++) {
							this->original_local_state_weights[c_index][it->first] = weights(s_index);
							s_index++;
						}
					}
					...
					for (int c_index = 0; c_index < (int)this->branch_node->branch_scope_context.size(); c_index++) {
						for (map<State*, vector<double>>::iterator it = p_temp_state_vals[c_index].begin();
								it != p_temp_state_vals[c_index].end(); it++) {
							this->original_temp_state_weights[c_index][it->first] = weights(s_index);
							s_index++;
						}
					}
				}
				...
				for (int i_index = 0; i_index < solution->curr_num_datapoints; i_index++) {
					obs_experiment_target_vals[i_index] = this->i_target_val_histories[i_index] - this->original_average_score;
				}
			}
