...
void SeedExperiment::measure_activate(AbstractNode*& curr_node,
									  Problem* problem,
...
void SeedExperiment::measure_back_activate(
		vector<ContextLayer>& context,
		RunHelper& run_helper) {
...
void SeedExperiment::measure_backprop(
		double target_val,
		RunHelper& run_helper) {
	if (run_helper.exceeded_limit) {
		...
		if (this->sub_state_iter >= NUM_DATAPOINTS
			...
			if (this->combined_score >= this->existing_average_score) {
				...
				this->state = SEED_EXPERIMENT_STATE_VERIFY_EXISTING;
				this->state_iter = 0;
			} else {
				this->result = EXPERIMENT_RESULT_FAIL;
			}
