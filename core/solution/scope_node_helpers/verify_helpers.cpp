...
void ScopeNode::verify_activate(AbstractNode*& curr_node,
	...
	vector<double> verify_input_input_state_vals;
	map<int, StateStatus> local_state_vals;
	vector<double> verify_input_local_state_vals;
		...
		if (this->input_types[i_index] == INPUT_TYPE_STATE) {
			if (this->input_outer_is_local[i_index]) {
				...
				if (this->input_inner_is_local[i_index]) {
					...
					verify_input_local_state_vals.push_back(state_status.val);
				} else {
					input_state_vals[this->input_inner_indexes[i_index]] = state_status;
					verify_input_input_state_vals.push_back(state_status.val);
				}
			} else {
				...
				if (it != context.back().input_state_vals.end()) {
					if (this->input_inner_is_local[i_index]) {
						...
						verify_input_local_state_vals.push_back(it->second.val);
					} else {
						input_state_vals[this->input_inner_indexes[i_index]] = it->second;
						verify_input_input_state_vals.push_back(it->second.val);
					}
				}
			}
		} else {
			if (this->input_inner_is_local[i_index]) {
				...
				input_state_vals[this->input_inner_indexes[i_index]] = StateStatus(this->input_init_vals[i_index],
																				   this->input_init_index_vals[i_index]);
				verify_input_input_state_vals.push_back(this->input_init_vals[i_index]);
			}
		}
	}
