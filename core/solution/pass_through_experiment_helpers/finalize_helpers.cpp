...
void PassThroughExperiment::new_branch(Solution* duplicate) {
	...
	this->info_branch_node->parent = duplicate_local_scope;
	duplicate_local_scope->nodes[this->info_branch_node->id] = this->info_branch_node;
	...
	this->info_branch_node->branch_end_node_id = new_ending_node->id;
	this->info_branch_node->branch_end_node = new_ending_node;
	...
	case NODE_TYPE_ACTION:
		{
			...
			if (action_node->next_node == this->best_exit_next_node) {
				this->info_branch_node->original_next_node_id = new_ending_node->id;
				this->info_branch_node->original_next_node = new_ending_node;
			} else {
				this->info_branch_node->original_next_node_id = action_node->next_node_id;
				this->info_branch_node->original_next_node = duplicate_local_scope->nodes[action_node->next_node_id];
			}
		}
		break;
	case NODE_TYPE_SCOPE:
		{
			...
			if (scope_node->next_node == this->best_exit_next_node) {
				...
				this->info_branch_node->original_next_node = duplicate_local_scope->nodes[scope_node->next_node_id];
			}
		}
		break;
	case NODE_TYPE_BRANCH:
		{
			...
			if (this->is_branch) {
				if (branch_node->branch_next_node == this->best_exit_next_node) {
					...
					this->info_branch_node->original_next_node = duplicate_local_scope->nodes[branch_node->branch_next_node_id];
				}
			} else {
				if (branch_node->original_next_node == this->best_exit_next_node) {
					...
					this->info_branch_node->original_next_node = duplicate_local_scope->nodes[branch_node->original_next_node_id];
				}
			}
		}
		break;
	case NODE_TYPE_INFO_BRANCH:
		{
			...
			if (this->is_branch) {
				if (info_branch_node->branch_next_node == this->best_exit_next_node) {
					...
					this->info_branch_node->original_next_node = duplicate_local_scope->nodes[info_branch_node->branch_next_node_id];
				}
			} else {
				if (info_branch_node->original_next_node == this->best_exit_next_node) {
					...
					this->info_branch_node->original_next_node = duplicate_local_scope->nodes[info_branch_node->original_next_node_id];
				}
			}
		}
		break;
	case NODE_TYPE_BRANCH_END:
		{
			...
			if (branch_end_node->next_node == this->best_exit_next_node) {
				...
				this->info_branch_node->original_next_node_id = branch_end_node->next_node_id;
				this->info_branch_node->original_next_node = duplicate_local_scope->nodes[branch_end_node->next_node_id];
			}
		}
		break;
	}
	...
	if (this->best_step_types.size() == 0) {
		this->info_branch_node->branch_next_node_id = new_ending_node->id;
		this->info_branch_node->branch_next_node = new_ending_node;
	} else {
	...
	case NODE_TYPE_ACTION:
		{
			...
			action_node->next_node_id = this->info_branch_node->id;
			action_node->next_node = this->info_branch_node;
		}
		break;
	case NODE_TYPE_SCOPE:
		{
			...
			scope_node->next_node_id = this->info_branch_node->id;
			scope_node->next_node = this->info_branch_node;
		}
		break;
	case NODE_TYPE_BRANCH:
		{
			...
			if (this->is_branch) {
				branch_node->branch_next_node_id = this->info_branch_node->id;
				branch_node->branch_next_node = this->info_branch_node;
			} else {
				branch_node->original_next_node_id = this->info_branch_node->id;
				branch_node->original_next_node = this->info_branch_node;
			}
		}
		break;
	case NODE_TYPE_INFO_BRANCH:
		{
			...
			if (this->is_branch) {
				info_branch_node->branch_next_node_id = this->info_branch_node->id;
				info_branch_node->branch_next_node = this->info_branch_node;
			} else {
				info_branch_node->original_next_node_id = this->info_branch_node->id;
				info_branch_node->original_next_node = this->info_branch_node;
			}
		}
		break;
	case NODE_TYPE_BRANCH_END:
		{
			...
			branch_end_node->next_node_id = this->info_branch_node->id;
			branch_end_node->next_node = this->info_branch_node;
		}
		break;
	}
	...
	new_ending_node->branch_start_node_id = this->info_branch_node->id;
	new_ending_node->branch_start_node = this->info_branch_node;
