...
void new_random_scope(SolutionNode* explore_node,
	...
	if (explore_node->parent_scope->node_type == NODE_TYPE_START_SCOPE) {
		...
		parent_jump_end_non_inclusive_index = \
			...
			+ rand()%((int)parent_start->path.size() - explore_node->scope_node_index);
	} else {
		...
		if (explore_node->scope_location == SCOPE_LOCATION_TOP) {
			parent_jump_end_non_inclusive_index = \
				explore_node->scope_node_index + 1 \
				+ rand()%((int)parent_jump->top_path.size() - explore_node->scope_node_index);
		} else {
			parent_jump_end_non_inclusive_index = \
				explore_node->scope_node_index + 1 \
	...
	if (seq_length == 0) {
		...
		for (int a_index = 0; a_index < seq_length; a_index++) {
			if (rand()%2 == 0 && action_dictionary->actions.size() > 0) {
				...
				int subset_non_inclusive_end = subset_inclusive_start + 1 + rand()%((int)existing_action.size() - subset_inclusive_start);
				for (int s_index = subset_inclusive_start; s_index < subset_non_inclusive_end; s_index++) {
					if (existing_action[s_index]->node_type == NODE_TYPE_EMPTY) {
						// if on top path, don't copy empty nodes, so do nothing
					} if (existing_action[s_index]->node_type == NODE_TYPE_ACTION) {
...
void get_existing_path(SolutionNode* explore_node,
	...
	if (explore_node->parent_scope->node_type == NODE_TYPE_START_SCOPE) {
		...
		for (int n_index = explore_node->parent_jump_scope_start_non_inclusive_index+1;
			...
			existing_path.push_back(parent_start->path[n_index]);
		}
	} else {
		// explore_node->parent_scope->node_type == NODE_TYPE_JUMP_SCOPE
		JumpScope* parent_jump = (JumpScope*)explore_node->parent_scope;
		if (explore_node->scope_location == SCOPE_LOCATION_TOP) {
			for (int n_index = explore_node->parent_jump_scope_start_non_inclusive_index+1;
					n_index <= explore_node->scope_node_index; n_index++) {
				existing_path.push_back(parent_jump->top_path[n_index]);
			}
		} else {
...
bool is_after_explore(vector<SolutionNode*>& current_scope,
	...
	if (current_scope_locations[explore_scope.size()-1] >= explore_parent_jump_end_non_inclusive_index) {
		return true;
	} else {
		return false;
	}
