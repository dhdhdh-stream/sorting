...
const int CURR_FILTER_EXCEED_LIMIT_LIMIT = 2;
#else
const int NUM_SAMPLES_PER_ITER = 20;
const int CURR_FILTER_EXCEED_LIMIT_LIMIT = 20;
#endif /* MDEBUG */

void SeedExperiment::find_filter_backprop(double target_val,
										  RunHelper& run_helper,
										  SeedExperimentOverallHistory* history) {
	if (!history->has_target && run_helper.exceeded_limit) {
		/**
		 * - possible seed path is an infinite loop if always taken
		 *   - simply give up if so
		 */
		this->curr_filter_exceeded_limit_count++;
		if (this->curr_filter_exceeded_limit_count >= CURR_FILTER_EXCEED_LIMIT_LIMIT) {
