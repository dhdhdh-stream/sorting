...
void SeedExperiment::find_gather_existing_backprop(
		double target_val,
	...
	if ((int)this->o_target_val_histories.size() >= FIND_GATHER_NUM_SAMPLES_PER_ITER) {
		double sum_scores = 0.0;
		for (int d_index = 0; d_index < FIND_GATHER_NUM_SAMPLES_PER_ITER; d_index++) {
			sum_scores += this->o_target_val_histories[d_index];
		}
		this->curr_gather_existing_average_score = sum_scores / FIND_GATHER_NUM_SAMPLES_PER_ITER;
		...
		this->curr_gather_existing_score_standard_deviation = sqrt(sum_score_variance / FIND_GATHER_NUM_SAMPLES_PER_ITER);
		...
		if (rand()%2 == 0) {
		#else
		this->curr_gather_seed_score /= FIND_GATHER_NUM_SAMPLES_PER_ITER;
		double new_higher_ratio = (double)this->curr_gather_is_higher / (double)FIND_GATHER_NUM_SAMPLES_PER_ITER;
		this->curr_gather_non_seed_score /= FIND_GATHER_NUM_SAMPLES_PER_ITER;

		double seed_score_diff = this->curr_gather_seed_score - this->curr_filter_score;
		double seed_t_score = seed_score_diff
		/ (this->existing_score_standard_deviation / sqrt(FIND_GATHER_NUM_SAMPLES_PER_ITER));
		...
		double ratio_t_score = (new_higher_ratio - this->curr_higher_ratio)
			/ (ratio_standard_deviation / sqrt(FIND_GATHER_NUM_SAMPLES_PER_ITER));
		...
		if (seed_t_score > -0.1 && ratio_t_score > -0.1 && non_seed_t_score > -0.1) {
			...
			this->state = SEED_EXPERIMENT_STATE_VERIFY_1ST_GATHER_SEED;
			/**
