		...
		case NODE_TYPE_BRANCH:
			{
				...
				for (int i_index = 0; i_index < (int)original_branch_node->original_input_scope_contexts.size(); i_index++) {
					new_branch_node->original_input_scope_context_ids.push_back(vector<int>());
					new_branch_node->original_input_scope_contexts.push_back(vector<Scope*>());
					new_branch_node->original_input_node_context_ids.push_back(vector<int>());
					new_branch_node->original_input_node_contexts.push_back(vector<AbstractNode*>());
					new_branch_node->original_input_obs_indexes.push_back(-1);
				}
				for (int i_index = 0; i_index < (int)original_branch_node->branch_input_scope_contexts.size(); i_index++) {
					new_branch_node->branch_input_scope_context_ids.push_back(vector<int>());
					new_branch_node->branch_input_scope_contexts.push_back(vector<Scope*>());
					new_branch_node->branch_input_node_context_ids.push_back(vector<int>());
					new_branch_node->branch_input_node_contexts.push_back(vector<AbstractNode*>());
					new_branch_node->branch_input_obs_indexes.push_back(-1);
				}
			...
			case NODE_TYPE_BRANCH:
				{
					...
					for (int i_index = 0; i_index < (int)original_branch_node->original_input_scope_contexts.size(); i_index++) {
						if (original_branch_node->original_input_scope_contexts[i_index].size() == 0) {
							new_branch_node->original_input_scope_context_ids.push_back(vector<int>());
							new_branch_node->original_input_scope_contexts.push_back(vector<Scope*>());
							new_branch_node->original_input_node_context_ids.push_back(vector<int>());
							new_branch_node->original_input_node_contexts.push_back(vector<AbstractNode*>());
							...
							map<AbstractNode*, AbstractNode*>::iterator it = node_mappings
								.find(original_branch_node->original_input_node_contexts[i_index][0]);
							if (it == node_mappings.end()) {
								new_branch_node->original_input_scope_context_ids.push_back(vector<int>());
								new_branch_node->original_input_scope_contexts.push_back(vector<Scope*>());
								new_branch_node->original_input_node_context_ids.push_back(vector<int>());
								...
								new_branch_node->original_input_scope_context_ids.push_back(original_branch_node->original_input_scope_context_ids[i_index]);
								new_branch_node->original_input_scope_context_ids[i_index][0] = new_scope->id;

								new_branch_node->original_input_node_context_ids.push_back(original_branch_node->original_input_node_context_ids[i_index]);
								new_branch_node->original_input_node_context_ids[i_index][0] = it->second->id;

								vector<Scope*> scope_context;
								vector<AbstractNode*> node_context;
								for (int l_index = 0; l_index < (int)new_branch_node->original_input_scope_context_ids[i_index].size(); l_index++) {
									Scope* duplicate_scope = duplicate->scopes[new_branch_node->original_input_scope_context_ids[i_index][l_index]];
									AbstractNode* duplicate_node = duplicate_scope->nodes[new_branch_node->original_input_node_context_ids[i_index][l_index]];
								...
								new_branch_node->original_input_obs_indexes.push_back(original_branch_node->original_input_obs_indexes[i_index]);
							}
						}
					}
					for (int i_index = 0; i_index < (int)original_branch_node->branch_input_scope_contexts.size(); i_index++) {
						if (original_branch_node->branch_input_scope_contexts[i_index].size() == 0) {
							new_branch_node->branch_input_scope_context_ids.push_back(vector<int>());
							new_branch_node->branch_input_scope_contexts.push_back(vector<Scope*>());
							new_branch_node->branch_input_node_context_ids.push_back(vector<int>());
							new_branch_node->branch_input_node_contexts.push_back(vector<AbstractNode*>());
							...
							map<AbstractNode*, AbstractNode*>::iterator it = node_mappings
								.find(original_branch_node->branch_input_node_contexts[i_index][0]);
							if (it == node_mappings.end()) {
								new_branch_node->branch_input_scope_context_ids.push_back(vector<int>());
								new_branch_node->branch_input_scope_contexts.push_back(vector<Scope*>());
								new_branch_node->branch_input_node_context_ids.push_back(vector<int>());
								...
								new_branch_node->branch_input_scope_context_ids.push_back(original_branch_node->branch_input_scope_context_ids[i_index]);
								new_branch_node->branch_input_scope_context_ids[i_index][0] = new_scope->id;

								new_branch_node->branch_input_node_context_ids.push_back(original_branch_node->branch_input_node_context_ids[i_index]);
								new_branch_node->branch_input_node_context_ids[i_index][0] = it->second->id;

								vector<Scope*> scope_context;
								vector<AbstractNode*> node_context;
								for (int l_index = 0; l_index < (int)new_branch_node->branch_input_scope_context_ids[i_index].size(); l_index++) {
									Scope* duplicate_scope = duplicate->scopes[new_branch_node->branch_input_scope_context_ids[i_index][l_index]];
									AbstractNode* duplicate_node = duplicate_scope->nodes[new_branch_node->branch_input_node_context_ids[i_index][l_index]];
								...
								new_branch_node->branch_input_obs_indexes.push_back(original_branch_node->branch_input_obs_indexes[i_index]);
							}
						}
					}
