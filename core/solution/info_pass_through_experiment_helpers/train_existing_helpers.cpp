...
void InfoPassThroughExperiment::train_existing_activate(
...
bool InfoPassThroughExperiment::train_existing_info_back_activate(
...
void InfoPassThroughExperiment::train_existing_back_activate(
		vector<ContextLayer>& context,
		RunHelper& run_helper) {
	InfoPassThroughExperimentHistory* history = (InfoPassThroughExperimentHistory*)run_helper.experiment_histories.back();
...
void InfoPassThroughExperiment::train_existing_backprop(
		double target_val,
		RunHelper& run_helper) {
	InfoPassThroughExperimentHistory* history = (InfoPassThroughExperimentHistory*)run_helper.experiment_histories.back();
	...
	if ((int)this->target_val_histories.size() >= NUM_DATAPOINTS
		...
		this->state = INFO_PASS_THROUGH_EXPERIMENT_STATE_TRAIN_NEW;
		this->state_iter = 0;
	}
}
