...
void BranchExperiment::train_existing_backprop(
	...
	if (this->score_type == SCORE_TYPE_FINAL) {
		if (this->state_iter >= FINAL_MIN_NUM_RUNS
				&& (int)this->target_val_histories.size() >= NUM_DATAPOINTS) {
			is_done = true;
		}
	} else {
	...
	if (is_done) {
		...
		int train_index = 0;
		while (train_index < 3) {
			vector<vector<Scope*>> test_input_scope_contexts = this->existing_input_scope_contexts;
			...
			if (is_select) {
				...
				for (int i_index = test_input_size-1; i_index >= original_input_size; i_index--) {
					vector<vector<Scope*>> remove_test_input_scope_contexts = this->existing_input_scope_contexts;
