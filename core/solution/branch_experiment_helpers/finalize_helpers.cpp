...
void BranchExperiment::finalize(map<pair<int, pair<bool,int>>, int>& input_scope_depths_mappings,
	...
	if (this->best_is_loop) {
		new_loop(input_scope_depths_mappings,
				 output_scope_depths_mappings);
	} else if (this->is_pass_through) {
...
void BranchExperiment::new_loop(map<pair<int, pair<bool,int>>, int>& input_scope_depths_mappings,
								map<pair<int, pair<bool,int>>, int>& output_scope_depths_mappings) {
	...
	if (this->best_exit_node == NULL) {
		...
		new_scope_node->next_node_id = this->best_exit_node->id;
	}
	new_scope_node->next_node = this->best_exit_node;

	delete this->best_potential_scopes[0];
