...
void EvalExperiment::train_existing_helper(vector<double>& existing_target_vals) {
	...
	gather_possible_helper(scope_context,
						   ...
						   this->existing_decision_scope_histories.back());
	...
	int num_obs = min(LINEAR_NUM_OBS, (int)possible_node_contexts.size());
	{
		...
		for (int o_index = 0; o_index < num_obs; o_index++) {
			...
			this->decision_input_node_contexts.push_back(possible_node_contexts[remaining_indexes[rand_index]]);
			this->decision_input_obs_indexes.push_back(possible_obs_indexes[remaining_indexes[rand_index]]);
	...
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
			int curr_layer = 0;
			ScopeHistory* curr_scope_history = this->existing_decision_scope_histories[d_index];
			while (true) {
				...
				if (it == curr_scope_history->node_histories.end()) {
					...
					if (curr_layer == (int)this->decision_input_node_contexts[i_index].size()-1) {
						if (it->first->type == NODE_TYPE_ACTION) {
							...
							inputs(d_index, i_index) = action_node_history->obs_snapshot[this->decision_input_obs_indexes[i_index]];
						} else {
	...
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		outputs(d_index) = existing_target_vals[d_index] - this->existing_average_score;
	}
	...
	try {
		...
		weights = Eigen::VectorXd(this->decision_input_node_contexts.size());
		for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
			weights(i_index) = 0.0;
		}
	}
	this->existing_linear_weights = vector<double>(this->decision_input_node_contexts.size());
	for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
		...
		if (abs(weights(i_index)) * average_impact < WEIGHT_MIN_SCORE_IMPACT * this->original_misguess_standard_deviation
	...
	this->existing_network_average_misguess = sum_misguesses / NUM_DATAPOINTS;

	double sum_misguess_variances = 0.0;
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		sum_misguess_variances += (misguesses[d_index] - this->existing_network_average_misguess) * (misguesses[d_index] - this->existing_network_average_misguess);
	}
	this->existing_network_misguess_standard_deviation = sqrt(sum_misguess_variances / NUM_DATAPOINTS);
	if (this->existing_network_misguess_standard_deviation < MIN_STANDARD_DEVIATION) {
		this->existing_network_misguess_standard_deviation = MIN_STANDARD_DEVIATION;
	}
	...
	while (train_index < 3) {
		...
		uniform_int_distribution<int> history_distribution(0, (int)this->existing_decision_scope_histories.size()-1);
		...
		gather_possible_helper(scope_context,
							   ...
							   possible_node_contexts,
							   possible_obs_indexes,
							   this->existing_decision_scope_histories[history_index]);
		...
		if (rand()%3 == 0) {
		#else
		double improvement = this->existing_network_average_misguess - average_misguess;
		double standard_deviation = min(this->existing_network_misguess_standard_deviation, misguess_standard_deviation);
		...
		if (t_score > 1.645) {
			...
			for (int t_index = 0; t_index < (int)test_network_input_scope_contexts.size(); t_index++) {
				int index = -1;
				for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
					if (test_network_input_node_contexts[t_index] == this->decision_input_node_contexts[i_index]
							&& test_network_input_obs_indexes[t_index] == this->decision_input_obs_indexes[i_index]) {
						index = i_index;
						break;
					}
				}
				if (index == -1) {
					this->decision_input_node_contexts.push_back(test_network_input_node_contexts[t_index]);
					this->decision_input_obs_indexes.push_back(test_network_input_obs_indexes[t_index]);
			...
			this->existing_network_average_misguess = average_misguess;
			this->existing_network_misguess_standard_deviation = misguess_standard_deviation;
