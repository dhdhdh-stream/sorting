...
void EvalExperiment::capture_existing_backprop(
	...
	if (this->existing_decision_scope_histories.size() >= NUM_DATAPOINTS) {
		...
		if (solution->eval->eval_network == NULL) {
			for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
				misguesses[d_index] = (this->existing_target_val_histories[d_index] - solution->eval->average_score) * (this->existing_target_val_histories[d_index] - solution->eval->average_score);
			}
		} else {
			for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
				vector<double> input_vals(solution->eval->input_scope_contexts.size(), 0.0);
				for (int i_index = 0; i_index < (int)solution->eval->input_scope_contexts.size(); i_index++) {
					int curr_layer = 0;
					...
					while (true) {
						...
						if (it == curr_scope_history->node_histories.end()) {
							...
							if (curr_layer == (int)solution->eval->input_scope_contexts[i_index].size()-1) {
