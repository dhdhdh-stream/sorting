...
void EvalExperiment::train_new_helper(vector<double>& new_target_vals) {
	...
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
			int curr_layer = 0;
			ScopeHistory* curr_scope_history = this->new_decision_scope_histories[d_index];
			while (true) {
	...
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		outputs(d_index) = new_target_vals[d_index] - this->new_average_score;
	}
	...
	this->new_linear_weights = vector<double>(this->decision_input_node_contexts.size());
	for (int i_index = 0; i_index < (int)this->decision_input_node_contexts.size(); i_index++) {
	...
	this->new_network_average_misguess = sum_misguesses / NUM_DATAPOINTS;

	double sum_misguess_variances = 0.0;
	for (int d_index = 0; d_index < NUM_DATAPOINTS; d_index++) {
		sum_misguess_variances += (misguesses[d_index] - this->new_network_average_misguess) * (misguesses[d_index] - this->new_network_average_misguess);
	}
	this->new_network_misguess_standard_deviation = sqrt(sum_misguess_variances / NUM_DATAPOINTS);
	if (this->new_network_misguess_standard_deviation < MIN_STANDARD_DEVIATION) {
		this->new_network_misguess_standard_deviation = MIN_STANDARD_DEVIATION;
	}
	...
	while (train_index < 3) {
		...
		gather_possible_helper(scope_context,
							   ...
							   this->new_decision_scope_histories[history_index]);
		...
		if (rand()%3 == 0) {
		#else
		double improvement = this->new_network_average_misguess - average_misguess;
		double standard_deviation = min(this->new_network_misguess_standard_deviation, misguess_standard_deviation);
		...
		if (t_score > 1.645) {
			...
			this->new_network_average_misguess = average_misguess;
			this->new_network_misguess_standard_deviation = misguess_standard_deviation;
