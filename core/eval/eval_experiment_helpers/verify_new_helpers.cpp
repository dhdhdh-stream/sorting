...
void EvalExperiment::verify_new_activate(
...
void EvalExperiment::verify_new_back_activate(
		vector<ContextLayer>& context,
		EvalExperimentHistory* history) {
...
void EvalExperiment::verify_new_backprop(double target_val,
										 RunHelper& run_helper) {
	...
	if (this->state == EVAL_EXPERIMENT_STATE_VERIFY_1ST
		...
		if (rand()%2 == 0) {
		#else
		if (this->combined_misguess < this->original_average_misguess) {
		#endif /* MDEBUG */
			this->original_average_misguess = 0.0;

			this->state = EVAL_EXPERIMENT_STATE_VERIFY_2ND_EXISTING;
			this->state_iter = 0;
		} else {
		...
		if (this->combined_misguess < this->original_average_misguess) {
			...
			for (int a_index = 0; a_index < (int)this->actions.size(); a_index++) {
				cout << " " << this->actions[a_index]->action.move;
			}
			cout << endl;

			cout << "this->combined_misguess: " << this->combined_misguess << endl;
			cout << "this->original_average_misguess: " << this->original_average_misguess << endl;

			cout << "this->is_pass_through: " << this->is_pass_through << endl;
