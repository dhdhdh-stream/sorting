...
AllToAllNetwork::AllToAllNetwork(int num_inputs) {
	...
	for (int n_index = 0; n_index < 20; n_index++) {
	...
	for (int o_index = 0; o_index < num_inputs; o_index++) {
...
AllToAllNetwork::AllToAllNetwork(ifstream& input_file) {
	...
	this->input_to_hidden->load_weights_from(input_file);
	this->hidden->load_weights_from(input_file);
	this->hidden_to_output->load_weights_from(input_file);
...
void AllToAllNetwork::save(ofstream& output_file) {
	output_file << this->input->acti_vals.size() << endl;

	this->input_to_hidden->save_weights(output_file);
	this->hidden->save_weights(output_file);
	this->hidden_to_output->save_weights(output_file);
