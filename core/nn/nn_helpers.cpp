...
void train_network(vector<vector<vector<double>>>& inputs,
				   ...
				   vector<bool>& test_input_is_fuzzy_match,
				   vector<int>& test_input_strict_root_indexes,
				   Network* network) {
	...
	for (int i_index = num_new_inputs-1; i_index >= 0; i_index--) {
		...
		if (input_impacts[i_index] < max_impact * NETWORK_INPUT_MIN_IMPACT) {
			...
			test_input_is_fuzzy_match.erase(test_input_is_fuzzy_match.begin() + i_index);
			test_input_strict_root_indexes.erase(test_input_strict_root_indexes.begin() + i_index);
