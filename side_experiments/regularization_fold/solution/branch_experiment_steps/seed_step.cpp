...
void BranchExperiment::seed_pre_activate_helper(vector<double>& new_state_vals,
												bool can_zero,
	...
	// simply don't worry about initializing networks for seed
	...
	for (int i_index = 0; i_index < (int)scope_history->node_histories.size(); i_index++) {
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
					...
					for (int s_index = 0; s_index < NUM_NEW_STATES; s_index++) {
						if (can_zero && rand()%5 == 0) {
							...
							network->new_activate(obs_snapshots.back(),
												  ...
												  network_history);
							new_state_network_histories.back()[s_index] = network_history;
				...
				seed_pre_activate_helper(new_state_vals,
										 can_zero,
										 obs_snapshots,
...
void BranchExperiment::seed_activate() {
	...
	this->starting_score_network->new_activate(this->seed_state_vals_snapshot,
											   new_state_vals);
	...
	for (int n_index = 0; n_index < (int)new_state_network_histories.size(); n_index++) {
		for (int s_index = 0; s_index < NUM_NEW_STATES; s_index++) {
			if (new_state_network_histories[n_index][s_index] != NULL) {
