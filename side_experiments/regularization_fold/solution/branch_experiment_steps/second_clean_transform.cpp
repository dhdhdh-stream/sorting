		...
		if (this->step_types[a_index] == BRANCH_EXPERIMENT_STEP_TYPE_SEQUENCE) {
			...
			for (int i_index = 0; i_index < input_size; i_index++) {
				...
				if (this->sequences[a_index]->input_types[i_index] == SEQUENCE_INPUT_TYPE_LOCAL
					...
					int new_family_id = continue_scope->state_family_ids[this->sequences[a_index]->input_local_input_indexes[i_index]];
					/**
					...
					int new_family_id = this->new_state_family_ids[new_scope_input_index];
					...
					if (this->sequences[a_index]->input_furthest_layer_needed_in[i_index] < this->scope_context.size()+2) {
						...
						if (this->sequences[a_index]->input_types[i_index] == SEQUENCE_INPUT_TYPE_NONE) {
							...
							new_family = solution->families[last_seen_class->family_id];
						}
			...
			for (int s_index = 0; s_index < NUM_NEW_STATES; s_index++) {
				if (this->step_state_networks[a_index][s_index] != NULL) {
					this->step_state_networks[a_index][s_index]->finalize_new_state(
						...
						this->num_new_states);
					this->new_action_node_target_indexes[a_index].push_back(this->num_new_states - this->layer_num_new_states.back() + s_index);
					this->new_action_node_state_networks[a_index].push_back(this->step_state_networks[a_index][s_index]);
				}
			}

			this->step_score_networks[a_index]->finalize_new_state(this->num_new_states);
		}
	}

	this->starting_score_network->finalize_new_state(this->scope_context.back()->num_states);
	this->starting_misguess_network->finalize_new_state(this->scope_context.back()->num_states);
