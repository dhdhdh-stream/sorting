...
void Sequence::new_class_outer_activate_helper(vector<double>& input_vals,
											   int context_index,
		...
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
				...
				if (i_index == (int)scope_history->node_histories.size()-1
					...
					new_class_outer_activate_helper(input_vals,
													context_index,
													run_helper,
...
void Sequence::new_class_experiment_activate_helper(vector<double>& input_vals,
...
void Sequence::new_class_activate(vector<double>& flat_vals,
	...
	new_class_experiment_activate_helper(input_vals,
	...
	for (int i_index = 0; i_index < (int)this->input_init_types.size(); i_index++) {
		if (this->input_init_types[i_index] == SEQUENCE_INPUT_INIT_NONE) {
			...
			if (this->input_is_new_type[i_index]) {
				double new_type_scale = (1000000.0-this->state_iter)/1000000.0;
	...
	for (int l_index = 0; l_index < (int)this->scopes.size(); l_index++) {
		for (int n_index = 0; n_index < (int)this->node_ids[l_index].size(); n_index++) {
			...
			// TODO: need to handle if is halfway activate
			if (n_index == (int)this->node_ids[l_index].size()-1 && l_index != (int)this->scopes.size()-1) {
				...
				// TODO: handle chaining state inwards and outwards
				// TODO: set next state_vals, but also construct input
				state_vals[l_index+1] = vector<double>(scopes[l_index+1]->num_states, 0.0);
				for (int i_index = 0; i_index < (int)scope_node->inner_input_indexes.back().size(); i_index++) {
					double val = state_vals[l_index][scope_node->inner_input_indexes.back()[i_index]];
					if (scope_node->inner_input_transformations.back()[i_index] != NULL) {
						val = scope_node->inner_input_transformations.back()[i_index]->forward(val);
					}
					state_vals[l_index+1][scope_node->inner_input_target_indexes.back()[i_index]] = val;
				}
	...
	if (this->scopes.size() > 1) {
		...
		for (int l_index = (int)this->scopes.size()-2; l_index >= 0; l_index--) {
			...
			if (scope_node->is_halfway) {
				for (int i_index = 0; i_index < (int)scope_node->halfway_link_indexes.size(); i_index++) {
					double val = state_vals[l_index+1][scope_node->halfway_link_target_indexes[i_index]];
					if (scope_node->halfway_link_transformations[i_index] != NULL) {
						val = scope_node->halfway_link_transformations[i_index]->backward(val);
					}
					state_vals[l_index][scope_node->halfway_link_indexes[i_index]] = val;
				}
			} else {
			...
			for (int i_index = 0; i_index < (int)scope_node->inner_input_indexes.back().size(); i_index++) {
				double val = state_vals[l_index+1][scope_node->inner_input_target_indexes.back()[i_index]];
				if (scope_node->inner_input_transformations.back()[i_index] != NULL) {
					val = scope_node->inner_input_transformations.back()[i_index]->backward(val);
				}
				state_vals[l_index][scope_node->inner_input_indexes.back()[i_index]] = val;
			}
	...
	for (int i_index = 0; i_index < (int)this->input_init_types.size(); i_index++) {
		if (this->input_init_types[i_index] == SEQUENCE_INPUT_INIT_LOCAL) {
			if (this->input_is_new_type[i_index]) {
				...
				double set_back_scale = (1000000.0-this->state_iter)/1000000.0;

				context[context.size()-1 - this->input_init_local_scope_depths[i_index]]
					.state_vals[this->input_init_local_input_indexes[i_index]] += set_back_scale*val_diff;
			} else {
				context[context.size()-1 - this->input_init_local_scope_depths[i_index]]
				...
				if (curr_sequence->input_init_types[curr_input_index] == SEQUENCE_INPUT_INIT_NONE) {
					...
					if (path_has_new_type) {
						double val_diff = input_vals[i_index] - context[context.size()-1 - curr_sequence->input_init_local_scope_depths[curr_input_index]]
							.state_vals[curr_sequence->input_init_local_input_indexes[curr_input_index]];

						double set_back_scale = (1000000.0-this->state_iter)/1000000.0;

						context[context.size()-1 - curr_sequence->input_init_local_scope_depths[curr_input_index]]
							.state_vals[curr_sequence->input_init_local_input_indexes[curr_input_index]] += set_back_scale*val_diff;
					} else {
						context[context.size()-1 - curr_sequence->input_init_local_scope_depths[curr_input_index]]
					...
					if (curr_sequence->input_is_new_type[curr_input_index]) {
						path_has_new_type = true;
					}
