...
void ActionNode::activate(vector<double>& flat_vals,
	...
	if (run_helper.explore_phase == EXPLORE_PHASE_EXPERIMENT) {
		...
		if (this->id < run_helper.scope_state_networks->size()
			...
			if (run_helper.experiment->state == EXPERIMENT_STATE_CLEANUP) {
				double temp_score_scale = (200000.0-run_helper.experiment->state_iter)/200000.0;
				run_helper.predicted_score += scale_factor*temp_score_scale*score_network->output->acti_vals[0];
			} else {
