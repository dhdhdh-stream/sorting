...
public:
	...
	// scope-less, i.e., go all the way back to the absolute outer scope?
	// which means that these are literally global
	// so the detection for specific state is uniform -- but whether to apply that detection is scope specific
	std::map<int, StateNetwork*> rewind_state_networks;
	// or should be scope-based, so have ability to adjust
	// or not scope-based, but may have dependencies, and those dependencies might be scope-based
