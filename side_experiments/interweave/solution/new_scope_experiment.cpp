...
NewScopeExperiment::NewScopeExperiment(Scope* scope_context,
	...
	if (this->new_scope != NULL) {
		...
		this->test_starts.push_back(node_context);
		this->test_is_branch.push_back(is_branch);
		this->test_exits.push_back(exit_next_node);
...
void NewScopeExperiment::decrement(AbstractNode* experiment_node) {
	...
	if (is_test) {
		this->test_starts.erase(this->test_starts.begin() + location_index);
		this->test_is_branch.erase(this->test_is_branch.begin() + location_index);
		this->test_exits.erase(this->test_exits.begin() + location_index);
		this->test_existing_scores.erase(this->test_existing_scores.begin() + location_index);
		this->test_existing_counts.erase(this->test_existing_counts.begin() + location_index);
		this->test_new_scores.erase(this->test_new_scores.begin() + location_index);
		this->test_new_counts.erase(this->test_new_counts.begin() + location_index);
	} else {
		this->successful_starts.erase(this->successful_starts.begin() + location_index);
		this->successful_is_branch.erase(this->successful_is_branch.begin() + location_index);
		this->successful_exits.erase(this->successful_exits.begin() + location_index);
	}

	bool still_needed = false;
	for (int t_index = (int)this->test_starts.size()-1; t_index >= 0; t_index--) {
		if (this->scope_context->nodes.find(this->test_starts[t_index]->id) != this->scope_context->nodes.end()) {
			still_needed = true;
			break;
		}
	}
	for (int s_index = (int)this->successful_starts.size()-1; s_index >= 0; s_index--) {
		if (this->scope_context->nodes.find(this->successful_starts[s_index]->id) != this->scope_context->nodes.end()) {
			still_needed = true;
			break;
		}
	}
