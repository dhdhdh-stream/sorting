		...
		if (this->run_iter >= TRAIN_NUM_DATAPOINTS) {
			...
			if (this->new_factor_ids.size() > 0) {
				...
				for (int i_index = 0; i_index < num_train_instances; i_index++) {
					sum_offset += abs(this->target_val_histories[i_index] - this->new_average_score);
				}
				...
				for (int i_index = 0; i_index < num_train_instances; i_index++) {
					outputs(i_index) = this->target_val_histories[i_index] - this->new_average_score;
				}
				...
				for (int i_index = 0; i_index < num_instances; i_index++) {
					remaining_scores[i_index] = this->target_val_histories[i_index] - this->new_average_score;
