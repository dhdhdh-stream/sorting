...
void clean_pre_signal_helper(double& existing_sum_misguess,
	...
	curr_signal->copy_from(scope->pre_signal,
						   wrapper->solution);
...
void clean_post_signal_helper(double& existing_sum_misguess,
							  Scope* scope,
							  int node_index,
							  SolutionWrapper* wrapper) {
	Signal* curr_signal = new Signal();
	curr_signal->copy_from(scope->post_signal,
						   wrapper->solution);
...
void update_post_signal(Scope* scope,
						SolutionWrapper* wrapper) {
	Signal* signal = scope->post_signal;
	...
	for (int n_index = (int)scope->post_signal->nodes.size()-1; n_index >= 0; n_index--) {
		clean_post_signal_helper(existing_sum_misguess,
								 scope,
								 n_index,
								 wrapper);
	}

	set_post_signal_potential_inputs(scope);
}
