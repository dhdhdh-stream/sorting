...
bool ExploreExperiment::predict_cycle(SolutionWrapper* wrapper) {
	...
	for (int e_index = 0; e_index < NUM_EXPLORE; e_index++) {
		...
		uniform_int_distribution<int> action_distribution(0, wrapper->solution->num_actions-1);
		...
		for (int s_index = 0; s_index < (int)curr_step_types.size(); s_index++) {
			if (curr_step_types[s_index] == STEP_TYPE_ACTION) {
				ActionNode* generic_action_node = wrapper->solution->generic_action_nodes[curr_indexes[s_index]];
				generic_action_node->predict_network->activate(state);
			} else {
				Scope* scope = this->scope_context->child_scopes[curr_indexes[s_index]];
				ScopeNode* generic_scope_node = wrapper->solution->generic_scope_nodes[scope->id];
				generic_scope_node->predict_network->activate(state);
			}
		}
		double predicted = predict_helper(state,
										  this->exit_next_node,
										  this->existing_node_context_histories[run_index][instance_index],
										  wrapper);
			...
			for (int i_index = 0; i_index < (int)this->existing_obs_histories[h_index].size(); i_index++) {
				...
				for (int s_index = 0; s_index < (int)this->best_step_types.size(); s_index++) {
					if (this->best_step_types[s_index] == STEP_TYPE_ACTION) {
						ActionNode* generic_action_node = wrapper->solution->generic_action_nodes[this->best_indexes[s_index]];
						generic_action_node->predict_network->activate(state);
					} else {
						Scope* scope = this->scope_context->child_scopes[this->best_indexes[s_index]];
						ScopeNode* generic_scope_node = wrapper->solution->generic_scope_nodes[scope->id];
						generic_scope_node->predict_network->activate(state);
					}
				}
				double predicted = predict_helper(state,
												  this->exit_next_node,
												  this->existing_node_context_histories[h_index][i_index],
												  wrapper);
