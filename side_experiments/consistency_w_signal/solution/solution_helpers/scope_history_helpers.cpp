...
void add_existing_samples_helper(ScopeHistory* scope_history,
								 vector<double>& sum_signals) {
	Scope* scope = scope_history->scope;

	switch (scope->signal_status) {
	...
	case SIGNAL_STATUS_ESTABLISHED:
		...
		sum_signals[scope->id] += scope_history->signal_val;
...
void add_explore_samples_helper(ScopeHistory* scope_history,
	...
	if (scope->signal_status == SIGNAL_STATUS_INIT) {
		double sum_vals = target_val - wrapper->solution->curr_score;
		for (int l_index = 0; l_index < (int)stack_trace.size(); l_index++) {
			if (stack_trace[l_index]->scope->signal_status == SIGNAL_STATUS_ESTABLISHED) {
