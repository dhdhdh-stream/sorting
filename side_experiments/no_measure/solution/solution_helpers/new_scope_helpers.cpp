		...
		for (map<AbstractNode*, AbstractNode*>::iterator node_it = node_mappings.begin();
			...
			case NODE_TYPE_BRANCH:
				if (node_it->first->type == node_it->second->type) {
					...
					new_branch_node->consec_original = original_branch_node->consec_original;
					new_branch_node->consec_branch = original_branch_node->consec_branch;
...
Scope* create_new_scope(Scope* scope_context,
	...
	if (scope_context->run_history.size() == 0) {
		return NULL;
	}

	uniform_int_distribution<int> run_distribution(0, scope_context->run_history.size()-1);
	for (int t_index = 0; t_index < CREATE_NEW_SCOPE_NUM_TRIES; t_index++) {
		int run_index = run_distribution(generator);
		if (scope_context->run_history[run_index].size() >= 2) {
			uniform_int_distribution<int> node_distribution(0, scope_context->run_history[run_index].size()-1);
			int start_index;
			int end_index;
			while (true) {
				start_index = node_distribution(generator);
				end_index = node_distribution(generator);
			...
			create_new_scope(scope_context->nodes[scope_context->run_history[run_index][start_index]],
							 scope_context->nodes[scope_context->run_history[run_index][end_index]],
							 potential_new_scope);
...
Scope* outer_create_new_scope(SolutionWrapper* wrapper) {
	while (true) {
		...
		if (parent_scope->run_history.size() == 0) {
			continue;
		}

		uniform_int_distribution<int> run_distribution(0, parent_scope->run_history.size()-1);
		for (int t_index = 0; t_index < CREATE_NEW_SCOPE_NUM_TRIES; t_index++) {
			int run_index = run_distribution(generator);
			int min_length = OUTER_PATH_MIN_RATIO * (double)parent_scope->run_history[run_index].size();
			uniform_int_distribution<int> length_distribution(min_length, parent_scope->run_history[run_index].size());
			int length = length_distribution(generator);
			uniform_int_distribution<int> start_distribution(0, parent_scope->run_history[run_index].size() - length);
			int start_index = start_distribution(generator);

			AbstractNode* potential_start_node = parent_scope->nodes[parent_scope->run_history[run_index][start_index]];
			AbstractNode* potential_end_node = parent_scope->nodes[parent_scope->run_history[run_index][start_index + length-1]];
