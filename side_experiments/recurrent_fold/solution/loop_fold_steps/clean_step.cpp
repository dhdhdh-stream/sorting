...
void Fold::clean_activate(vector<double>& local_state_vals,
	...
	while (true) {
		...
		if (iter_index > 7) {
			...
			// TODO: scale by absolute value of scale_diff?
...
void LoopFold::backprop(vector<double>& local_state_errors,
	...
	if (run_helper.explore_phase == EXPLORE_PHASE_FLAT) {
					...
					for (int n_index = (int)history->inner_state_network_histories[iter_index][f_index].size()-1; n_index >= 0; n_index--) {
						for (int i_index = this->curr_num_new_inner_states-1; i_index >= 0; i_index--) {
							if (history->inner_state_network_histories[iter_index][f_index][n_index][i_index] != NULL) {
								...
								state_network->new_outer_backprop_errors_with_no_weight_change(
		...
		if (history->state_iter_snapshot <= this->state_iter) {
			for (int iter_index = history->num_loop_iters-1; iter_index >= 0; iter_index--) {
				...
				if (iter_index != history->num_loop_iters-1) {
					this->curr_continue_score_network->new_sequence_backprop(
						target_val - predicted_score,
						0.002,
						history->continue_score_network_histories[iter_index]);
					...
					this->curr_continue_misguess_network->new_sequence_backprop(
						final_misguess - history->continue_misguess_val[iter_index],
						0.002,
						history->continue_misguess_network_histories[iter_index]);
				} else {
					this->curr_halt_score_network->new_sequence_backprop(
						target_val - predicted_score,
						0.002,
						history->halt_score_network_history);
					...
					this->curr_halt_misguess_network->new_sequence_backprop(
						final_misguess - history->halt_misguess_val,
						0.002,
						history->halt_misguess_network_history);
				}
