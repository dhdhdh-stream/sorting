...
void Minesweeper::print_obs() {
	for (int y_index = this->current_y + FIELD_OF_VIEW; y_index >= this->current_y - FIELD_OF_VIEW; y_index--) {
		for (int x_index = this->current_x - FIELD_OF_VIEW; x_index <= this->current_x + FIELD_OF_VIEW; x_index++) {
			if (x_index < 0
					|| x_index > WIDTH-1
					|| y_index < 0
					|| y_index > HEIGHT-1) {
...
int TypeMinesweeper::num_obs() {
	return (2 * FIELD_OF_VIEW + 1) * (2 * FIELD_OF_VIEW + 1);
}
