...
void BranchExperiment::finalize(Solution* duplicate) {
	if (this->result == EXPERIMENT_RESULT_SUCCESS) {
		...
		if (this->select_percentage == 1.0) {
			...
			for (int f_index = 0; f_index < (int)this->new_factors.size(); f_index++) {
				ObsNode* obs_node = (ObsNode*)duplicate_local_scope->nodes[this->new_factors[f_index].node_context.back()];
				Factor* factor = obs_node->factors[this->new_factors[f_index].factor_index];
