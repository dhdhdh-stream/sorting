...
void BranchExperiment::train_existing_activate(
	...
	vector<double> factor_vals(this->existing_factors.size());
...
void BranchExperiment::train_existing_backprop(
	...
	if (this->state_iter >= TRAIN_EXISTING_NUM_DATAPOINTS) {
		...
		if (this->existing_factors.size() > 0) {
			...
			Eigen::MatrixXd inputs(num_train_instances, this->existing_factors.size());
			for (int i_index = 0; i_index < num_train_instances; i_index++) {
				for (int f_index = 0; f_index < (int)this->existing_factors.size(); f_index++) {
			...
			try {
				...
				weights = Eigen::VectorXd(this->existing_factors.size());
				for (int f_index = 0; f_index < (int)this->existing_factors.size(); f_index++) {
					weights(f_index) = 0.0;
				}
			}
			...
			for (int f_index = (int)this->existing_factors.size() - 1; f_index >= 0; f_index--) {
				#if defined(MDEBUG) && MDEBUG
				if (rand()%2 == 0) {
				#else
				...
				#endif /* MDEBUG */
					this->existing_factors.erase(this->existing_factors.begin() + f_index);
		...
		if (new_t_score > 2.326) {
			...
			if (this->existing_inputs.size() > 0) {
				...
				if (this->node_context->type == NODE_TYPE_OBS) {
					...
					this->existing_factors.push_back(new_input);
