...
void BranchExperiment::existing_gather_activate(
	...
	for (int f_index = 0; f_index < GATHER_FACTORS_PER_ITER; f_index++) {
		...
		if (new_factor.scope_context.size() != 0) {
			bool is_existing = false;
			for (int i_index = 0; i_index < (int)this->existing_factors.size(); i_index++) {
				if (new_factor == this->existing_factors[i_index]) {
					is_existing = true;
					break;
				}
			}
			if (!is_existing) {
				this->existing_factors.push_back(new_factor);
			}
		}
	}
}
...
void BranchExperiment::existing_gather_backprop() {
	...
	if (this->state_iter >= GATHER_ITERS) {
		...
		while (this->existing_factors.size() > GATHER_ITERS * GATHER_FACTORS_PER_ITER) {
			uniform_int_distribution<int> remove_distribution(0, this->existing_factors.size()-1);
			int remove_index = remove_distribution(generator);
			this->existing_factors.erase(this->existing_factors.begin() + remove_index);
		}
