...
void BranchExperiment::train_new_activate(
	...
	if (this->num_instances_until_target <= 0) {
		...
		vector<double> factor_vals(this->new_factors.size());
...
void BranchExperiment::train_new_backprop(
	...
	if (this->state_iter >= TRAIN_NEW_NUM_DATAPOINTS) {
		...
		if (this->new_factors.size() > 0) {
			...
			Eigen::MatrixXd inputs(num_train_instances, this->new_factors.size());
			for (int i_index = 0; i_index < num_train_instances; i_index++) {
				for (int f_index = 0; f_index < (int)this->new_factors.size(); f_index++) {
			...
			try {
				...
				weights = Eigen::VectorXd(this->new_factors.size());
				for (int f_index = 0; f_index < (int)this->new_factors.size(); f_index++) {
					weights(f_index) = 0.0;
				}
			}
			...
			for (int f_index = (int)this->new_factors.size() - 1; f_index >= 0; f_index--) {
				#if defined(MDEBUG) && MDEBUG
				if (rand()%2 == 0) {
				#else
				...
				#endif /* MDEBUG */
					this->new_factors.erase(this->new_factors.begin() + f_index);
		...
		if (new_t_score > 2.326) {
			...
			if (this->new_inputs.size() > 0) {
				...
				if (this->node_context->type == NODE_TYPE_OBS) {
					...
					this->new_factors.push_back(new_input);
