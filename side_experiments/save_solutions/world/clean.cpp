...
vector<double> Clean::get_observations() {
	vector<double> obs;
...
void Clean::perform_action(int action) {
	switch (action) {
	case ACTION_LEFT:
		this->curr_index--;
...
Problem* Clean::copy_and_reset() {
	Clean* new_problem = new Clean();
...
Problem* TypeClean::get_problem() {
	return new Clean();
}

int TypeClean::num_obs() {
	return 2;
}

int TypeClean::num_possible_actions() {
	return 3;
}
