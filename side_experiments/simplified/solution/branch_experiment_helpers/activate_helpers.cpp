...
void BranchExperiment::backprop(double target_val,
	...
	case BRANCH_EXPERIMENT_STATE_MEASURE:
		...
		if (this->curr_new_scope_histories.size() < MEASURE_ITERS) {
			this->curr_new_scope_histories.push_back(wrapper->scope_histories[0]);
			this->curr_new_target_val_histories.push_back(target_val);
		} else {
			uniform_int_distribution<int> distribution(0, this->curr_new_scope_histories.size()-1);
			int random_index = distribution(generator);
			delete this->curr_new_scope_histories[random_index];
			this->curr_new_scope_histories[random_index] = wrapper->scope_histories[0];
			this->curr_new_target_val_histories[random_index] = target_val;
		}
