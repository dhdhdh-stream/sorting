...
void BranchExperiment::capture_verify_check_activate(SolutionWrapper* wrapper) {
	...
	double sum_vals = this->best_constant;

	for (int i_index = 0; i_index < (int)this->best_inputs.size(); i_index++) {
		double val;
		bool is_on;
		fetch_input_helper(scope_history,
						   this->best_inputs[i_index],
						   0,
						   val,
						   is_on);
		if (is_on) {
			double normalized_val = (val - this->best_input_averages[i_index]) / this->best_input_standard_deviations[i_index];
			sum_vals += this->best_weights[i_index] * normalized_val;
	...
	if (this->best_network != NULL) {
		vector<double> input_vals(this->best_network_inputs.size());
		vector<bool> input_is_on(this->best_network_inputs.size());
		for (int i_index = 0; i_index < (int)this->best_network_inputs.size(); i_index++) {
			double val;
			bool is_on;
			fetch_input_helper(scope_history,
							   this->best_network_inputs[i_index],
							   0,
							   val,
							   is_on);
		...
		this->best_network->activate(input_vals,
									input_is_on);
		sum_vals += this->best_network->output->acti_vals[0];

		cout << "this->best_network->output->acti_vals[0]: " << this->best_network->output->acti_vals[0] << endl;
	}
