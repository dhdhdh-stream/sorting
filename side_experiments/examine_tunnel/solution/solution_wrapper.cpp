...
SolutionWrapper::SolutionWrapper(std::string path,
	...
	string tunnel_is_null_line;
	getline(input_file, tunnel_is_null_line);
	bool tunnel_is_null = stoi(tunnel_is_null_line);
	if (tunnel_is_null) {
		this->tunnel = NULL;
	} else {
		this->tunnel = new Tunnel(input_file);
	}
...
void SolutionWrapper::save(string path,
	...
	output_file << (this->tunnel == NULL) << endl;
	if (this->tunnel != NULL) {
		this->tunnel->save(output_file);
	}
