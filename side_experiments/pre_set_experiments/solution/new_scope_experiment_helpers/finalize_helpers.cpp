...
void NewScopeExperiment::finalize(Solution* duplicate) {
	...
	if (this->exit_next_node == NULL) {
		ActionNode* new_local_ending_node = new ActionNode();
		...
		AbstractNode* duplicate_end_node = duplicate_local_scope->nodes[this->exit_next_node->id];

		new_scope_node->next_node_id = duplicate_end_node->id;
		new_scope_node->next_node = duplicate_end_node;
	}

	AbstractNode* duplicate_start_node = duplicate_local_scope->nodes[this->node_context->id];
