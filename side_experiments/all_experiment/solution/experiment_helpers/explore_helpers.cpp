...
void Experiment::explore_check_activate(
	...
	if (wrapper->is_explore
		...
		this->temp_exit_next_node = possible_exits[random_index];
		...
		if (this->temp_new_scope != NULL) {
			...
			this->temp_scopes.push_back(this->temp_new_scope);
		} else {
			bool is_in_place;
			if (this->temp_exit_next_node == this->node_context->next_node) {
				is_in_place = true;
			} else {
				is_in_place = false;
			}
			...
			for (int s_index = 0; s_index < new_num_steps; s_index++) {
				...
				if (is_scope) {
					...
					this->temp_scopes.push_back(this->node_context->parent->child_scopes[child_index]);
				} else {
					this->temp_step_types.push_back(STEP_TYPE_ACTION);

					this->temp_actions.push_back(-1);

					this->temp_scopes.push_back(NULL);
				}
			}
		}
		...
		if (new_norm_score / existing_norm_score >= MIN_PROTECT) {
			...
			this->temp_existing_predicted = this->existing_network->output->acti_vals[0];
		} else {
			...
			this->temp_step_types.clear();
			this->temp_actions.clear();
			this->temp_scopes.clear();
		}
	}
}
...
void Experiment::explore_step(vector<double>& obs,
	...
	if (experiment_state->step_index >= (int)this->temp_step_types.size()) {
		wrapper->node_context.back() = this->temp_exit_next_node;

		delete experiment_state;
		wrapper->experiment_context.back() = NULL;

		uniform_int_distribution<int> select_distribution(0, this->curr_explore_count);
		this->curr_explore_count++;
		if (select_distribution(generator) == 0) {
			this->curr_existing_predicted = this->temp_existing_predicted;
			if (this->curr_new_scope != NULL) {
				delete this->curr_new_scope;
			}
			this->curr_new_scope = this->temp_new_scope;
			this->temp_new_scope = NULL;
			this->curr_step_types = this->temp_step_types;
			this->curr_actions = this->temp_actions;
			this->curr_scopes = this->temp_scopes;
			this->curr_exit_next_node = this->temp_exit_next_node;
		}
		...
		if (this->temp_step_types[experiment_state->step_index] == STEP_TYPE_ACTION) {
			if (this->temp_actions[experiment_state->step_index] == -1) {
				is_next = true;
				fetch_action = true;

				wrapper->num_actions++;
			} else {
				action = this->temp_actions[experiment_state->step_index];
				is_next = true;
			...
			ScopeHistory* inner_scope_history = new ScopeHistory(this->temp_scopes[experiment_state->step_index]);
			wrapper->scope_histories.push_back(inner_scope_history);
			wrapper->node_context.push_back(this->temp_scopes[experiment_state->step_index]->nodes[0]);
...
void Experiment::explore_set_action(int action,
									ExperimentState* experiment_state) {
	this->temp_actions[experiment_state->step_index] = action;
...
void Experiment::result_explore_step(vector<double>& obs,
									 ...
									 ExperimentState* experiment_state) {
	if (experiment_state->step_index >= (int)this->temp_step_types.size()) {
		wrapper->result_node_context.back() = this->temp_exit_next_node;
		...
		if (this->temp_step_types[experiment_state->step_index] == STEP_TYPE_ACTION) {
			...
			wrapper->result_node_context.push_back(this->temp_scopes[experiment_state->step_index]->nodes[0]);
