...
void Scope::random_pre(AbstractNode* starting_node,
	...
	while (true) {
		...
		uniform_int_distribution<int> distribution(0, curr_node->ancestor_ids.size()-1);
		curr_node = this->nodes[curr_node->ancestor_ids[distribution(generator)]];
	}
}

void Scope::random_exit(AbstractNode* starting_node,
...
void Scope::random_path(vector<AbstractNode*>& path) {
