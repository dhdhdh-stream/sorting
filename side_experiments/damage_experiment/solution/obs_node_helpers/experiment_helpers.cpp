...
void ObsNode::experiment_step(vector<double>& obs,
	...
	if (this->protect_type == PROTECT_TYPE_START) {
		wrapper->is_protect = true;
	}
	...
	#endif /* MDEBUG */
	if (!wrapper->is_protect && damage_distribution(generator) == 0) {
		AbstractNode* starting_node = this->next_node;
		vector<AbstractNode*> possible_exits;
		this->parent->random_exit(
			starting_node,
			possible_exits);
		...
		if (history->damage_new_scope != NULL) {
			...
			bool is_in_place;
			if (exit_next_node == this->next_node) {
				is_in_place = true;
			} else {
				is_in_place = false;
			}
	...
	if (this->protect_type == PROTECT_TYPE_END) {
		wrapper->is_protect = false;
	}
