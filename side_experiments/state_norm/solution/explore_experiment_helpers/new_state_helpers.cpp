...
void ExploreExperiment::new_state_helper(SolutionWrapper* wrapper) {
	...
	for (int iter_index = 0; iter_index < NEW_STATE_TRAIN_ITERS; iter_index++) {
		...
		for (int d_index = 0; d_index < (int)this->dependencies.size(); d_index++) {
			if (this->new_dependencies_is_hit_histories[rand_index][d_index]) {
				if (iter_index < NEW_STATE_PARTIAL_START_ITERS
					...
					if (iter_index < NEW_STATE_PARTIAL_START_ITERS) {
						init_networks[d_index]->init_activate(this->new_dependencies_state_norms_histories[rand_index][d_index],
															  this->new_dependencies_state_histories[rand_index][d_index],
															  this->new_dependencies_new_state_norms_histories[rand_index][d_index],
															  new_state,
															  this->new_dependencies_obs_histories[rand_index][d_index]);
					} else {
						init_networks[d_index]->init_activate_w_drop(
							this->new_dependencies_state_norms_histories[rand_index][d_index],
							this->new_dependencies_state_histories[rand_index][d_index],
							this->new_dependencies_new_state_norms_histories[rand_index][d_index],
							new_state,
							this->new_dependencies_obs_histories[rand_index][d_index]);
		...
		new_network->init_activate(this->new_state_norms_histories[rand_index],
								   this->new_state_histories[rand_index],
								   this->new_new_state_norms_histories[rand_index],
								   new_state);
		...
		for (int d_index = (int)this->dependencies.size()-1; d_index >= 0; d_index--) {
			if (is_activate[d_index]) {
				init_networks[d_index]->init_backprop(this->new_dependencies_new_state_norms_histories[rand_index][d_index],
													  new_state_errors);
			}
		}
	...
	for (int h_index = num_existing_train; h_index < (int)this->existing_dependencies_is_hit_histories.size(); h_index++) {
		...
		for (int d_index = 0; d_index < (int)this->dependencies.size(); d_index++) {
			if (this->existing_dependencies_is_hit_histories[h_index][d_index]) {
				init_networks[d_index]->init_activate(this->existing_dependencies_state_norms_histories[h_index][d_index],
													  this->existing_dependencies_state_histories[h_index][d_index],
													  this->existing_dependencies_new_state_norms_histories[h_index][d_index],
													  new_state,
													  this->existing_dependencies_obs_histories[h_index][d_index]);
			}
		}
		new_network->init_activate(this->existing_state_norms_histories[h_index],
								   this->existing_state_histories[h_index],
								   this->existing_new_state_norms_histories[h_index],
								   new_state);
	...
	for (int h_index = num_new_train; h_index < (int)this->new_dependencies_is_hit_histories.size(); h_index++) {
		...
		for (int d_index = 0; d_index < (int)this->dependencies.size(); d_index++) {
			if (this->new_dependencies_is_hit_histories[h_index][d_index]) {
				init_networks[d_index]->init_activate(this->new_dependencies_state_norms_histories[h_index][d_index],
													  this->new_dependencies_state_histories[h_index][d_index],
													  this->new_dependencies_new_state_norms_histories[h_index][d_index],
													  new_state,
													  this->new_dependencies_obs_histories[h_index][d_index]);
			}
		}
		new_network->init_activate(this->new_state_norms_histories[h_index],
								   this->new_state_histories[h_index],
								   this->new_new_state_norms_histories[h_index],
								   new_state);
