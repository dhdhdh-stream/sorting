...
void ExploreExperiment::train_new_check_activate(vector<double>& obs,
	...
	if (wrapper->run_type == RUN_TYPE_EXPLORE) {
		...
		if (this->num_instances_until_target <= 0) {
			...
			vector<int> curr_dependencies_new_state_norms(this->dependencies.size());
			vector<Eigen::VectorXf> curr_dependencies_state_norms(this->dependencies.size());
			...
			for (int d_index = 0; d_index < (int)this->dependencies.size(); d_index++) {
				...
				curr_dependencies_new_state_norms[d_index] = num_actions -
					(wrapper->scope_histories.back()->num_actions - 1);
				curr_dependencies_state_norms[d_index] = state_norms;
			...
			history->dependencies_new_state_norms_histories.push_back(curr_dependencies_new_state_norms);
			history->dependencies_state_norms_histories.push_back(curr_dependencies_state_norms);
			...
			history->new_state_norms_histories.push_back(wrapper->num_actions
				- (wrapper->scope_histories.back()->num_actions - 1));
			history->state_norms_histories.push_back(wrapper->state_norms);
...
void ExploreExperiment::train_new_step(vector<double>& obs,
	...
	if (experiment_state->step_index >= (int)this->best_step_types.size()) {
		...
		if (this->best_step_types[experiment_state->step_index] == STEP_TYPE_ACTION) {
			...
			action_network->activate(wrapper->state_norms,
									 wrapper->state);
...
void ExploreExperiment::train_new_callback(vector<double>& obs,
	...
	obs_network->activate(wrapper->state_norms,
						  wrapper->state,
						  obs);
}
	...
	if (wrapper->run_type == RUN_TYPE_EXPLORE) {
		if (history->dependencies_is_hit_histories.size() > 0) {
			for (int i_index = 0; i_index < (int)history->dependencies_is_hit_histories.size(); i_index++) {
				...
				this->new_dependencies_new_state_norms_histories.push_back(history->dependencies_new_state_norms_histories[i_index]);
				this->new_dependencies_state_norms_histories.push_back(history->dependencies_state_norms_histories[i_index]);
				...
				this->new_new_state_norms_histories.push_back(history->new_state_norms_histories[i_index]);
				this->new_state_norms_histories.push_back(history->state_norms_histories[i_index]);
			...
			if (this->state_iter >= EXPERIMENT_NUM_DATAPOINTS) {
				...
				{
					default_random_engine generator_copy = generator;
					shuffle(this->new_dependencies_new_state_norms_histories.begin(), this->new_dependencies_new_state_norms_histories.end(), generator_copy);
				}
				{
					default_random_engine generator_copy = generator;
					shuffle(this->new_dependencies_state_norms_histories.begin(), this->new_dependencies_state_norms_histories.end(), generator_copy);
				}
				{
					default_random_engine generator_copy = generator;
				...
				{
					default_random_engine generator_copy = generator;
					shuffle(this->new_new_state_norms_histories.begin(), this->new_new_state_norms_histories.end(), generator_copy);
				}
				{
					default_random_engine generator_copy = generator;
					shuffle(this->new_state_norms_histories.begin(), this->new_state_norms_histories.end(), generator_copy);
				}
				{
					default_random_engine generator_copy = generator;
				...
				for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
					...
					new_network->activate(this->new_state_norms_histories[rand_index],
				...
				for (int h_index = num_existing_train; h_index < (int)this->existing_dependencies_is_hit_histories.size(); h_index++) {
					this->existing_network->activate(this->existing_state_norms_histories[h_index],
													 this->existing_state_histories[h_index]);
					new_network->activate(this->existing_state_norms_histories[h_index],
										  this->existing_state_histories[h_index]);
				...
				for (int h_index = num_new_train; h_index < (int)this->new_dependencies_is_hit_histories.size(); h_index++) {
					this->existing_network->activate(this->new_state_norms_histories[h_index],
													 this->new_state_histories[h_index]);
					new_network->activate(this->new_state_norms_histories[h_index],
										  this->new_state_histories[h_index]);
					if (new_network->output->acti_vals[0] >= this->existing_network->output->acti_vals[0]) {
