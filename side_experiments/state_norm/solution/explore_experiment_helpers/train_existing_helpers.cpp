...
void ExploreExperiment::train_existing_backprop(
	...
	for (int i_index = 0; i_index < (int)history->state_histories.size(); i_index++) {
		...
		this->existing_dependencies_new_state_norms_histories.push_back(history->dependencies_new_state_norms_histories[i_index]);
		this->existing_dependencies_state_norms_histories.push_back(history->dependencies_state_norms_histories[i_index]);
		...
		this->existing_new_state_norms_histories.push_back(history->new_state_norms_histories[i_index]);
		this->existing_state_norms_histories.push_back(history->state_norms_histories[i_index]);
	...
	if (this->state_iter >= EXPERIMENT_NUM_DATAPOINTS) {
		...
		{
			default_random_engine generator_copy = generator;
			shuffle(this->existing_dependencies_new_state_norms_histories.begin(), this->existing_dependencies_new_state_norms_histories.end(), generator_copy);
		}
		{
			default_random_engine generator_copy = generator;
			shuffle(this->existing_dependencies_state_norms_histories.begin(), this->existing_dependencies_state_norms_histories.end(), generator_copy);
		}
		{
			default_random_engine generator_copy = generator;
		...
		{
			default_random_engine generator_copy = generator;
			shuffle(this->existing_new_state_norms_histories.begin(), this->existing_new_state_norms_histories.end(), generator_copy);
		}
		{
			default_random_engine generator_copy = generator;
			shuffle(this->existing_state_norms_histories.begin(), this->existing_state_norms_histories.end(), generator_copy);
		}
		{
			default_random_engine generator_copy = generator;
		...
		for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
			...
			this->existing_network->activate(this->existing_state_norms_histories[rand_index],
											 this->existing_state_histories[rand_index]);
