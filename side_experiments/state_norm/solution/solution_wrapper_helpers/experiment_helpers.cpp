...
void SolutionWrapper::experiment_init(vector<double> obs) {
	...
	this->state_norms.resize(this->solution->num_states);
	this->state_norms.setConstant(0.0);
	this->state.resize(this->solution->num_states);
	this->state.setConstant(0.0);
	this->partial_state.resize(this->solution->num_states);
	this->partial_state.setConstant(0.0);
...
tuple<bool,bool,int> SolutionWrapper::experiment_step(vector<double> obs) {
	if (this->last_was_damage) {
		...
		obs_network->activate(this->state_norms,
							  this->state,
							  obs);
	...
	if (this->run_type == RUN_TYPE_DAMAGE) {
		...
		if (damage_distribution(generator) == 0) {
			...
			action_network->activate(this->state_norms,
									 this->state);
