...
void BranchNode::experiment_step(vector<double>& obs,
	...
	if (this->consec_original >= CONSEC_DEPRECATE_LIMIT) {
		...
		bool is_branch;
		this->original_network->activate(wrapper->state_norms,
										 wrapper->state);
		this->branch_network->activate(wrapper->state_norms,
									   wrapper->state);
		...
		if (is_branch) {
			...
			if (wrapper->run_type != RUN_TYPE_EXPLORE) {
				this->explore_branch_network->activate(wrapper->state_norms,
													   wrapper->partial_state);
			...
			if (wrapper->run_type != RUN_TYPE_EXPLORE) {
				this->explore_original_network->activate(wrapper->state_norms,
														 wrapper->partial_state);
