...
void Layer::calc_max_update(int input_index,
	...
	if (input_index == -1) {
		...
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			int layer_size = (int)this->input_layers[input_index]->acti_vals.size();
			for (int ln_index = 0; ln_index < layer_size; ln_index++) {
				double update = this->weight_updates[n_index][input_index][ln_index]
								*learning_rate
								+this->prev_weight_updates[n_index][input_index][ln_index]
								*momentum;
				double update_size = abs(update);
				if (update_size > max_update_size) {
					max_update_size = update_size;
		...
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			...
			for (int ln_index = 0; ln_index < layer_size; ln_index++) {
				double update = this->weight_updates[n_index][input_index][ln_index]
								...
								*momentum;
				update *= factor;
				this->weight_updates[n_index][input_index][ln_index] = 0.0;
				this->weights[n_index][input_index][ln_index] += update;
				this->prev_weight_updates[n_index][input_index][ln_index] = update;
			}
		}
	}
}
