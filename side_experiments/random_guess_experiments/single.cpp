...
int main(int argc, char* argv[]) {
	...
	Scope* curr_solution = new Scope(starting_nodes);
	...
	while (true) {
		...
		for (int p_index = 0; p_index < num_parts; p_index++) {
			...
			Scope* part = new Scope(curr_solution);
		...
		bool fail = false;
		int num_seen_spot_1 = 0;
		int num_seen_spot_3 = 0;
		int num_seen_spot_7 = 0;
		int num_seen_spot_9 = 0;
		bool sign = true;
		for (int s_index = 0; s_index < (int)spots.size(); s_index++) {
			if (spots[s_index] == 1) {
				num_seen_spot_1++;
			} else if (spots[s_index] == 3) {
				num_seen_spot_3++;
			} else if (spots[s_index] == 7) {
				num_seen_spot_7++;
			} else if (spots[s_index] == 9) {
				num_seen_spot_9++;
			} else {
				fail = true;
				break;
