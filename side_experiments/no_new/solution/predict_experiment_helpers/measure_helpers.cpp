...
void PredictExperiment::measure_check_activate(
		vector<double>& obs,
		PredictExperimentHistory* history,
		SolutionWrapper* wrapper) {
	if (wrapper->run_type == RUN_TYPE_EXPERIMENT) {
		bool is_branch;
		...
		if (is_branch) {
			PredictExperimentState* new_experiment_state = new PredictExperimentState(this);
...
void PredictExperiment::measure_step(vector<double>& obs,
									 int& action,
									 bool& is_next,
									 SolutionWrapper* wrapper) {
	PredictExperimentState* experiment_state = (PredictExperimentState*)wrapper->experiment_context.back();
...
void PredictExperiment::measure_callback(vector<double>& obs,
										 SolutionWrapper* wrapper) {
	PredictExperimentState* experiment_state = (PredictExperimentState*)wrapper->experiment_context.back();
...
void PredictExperiment::measure_exit_step(vector<double>& obs,
										  SolutionWrapper* wrapper) {
	PredictExperimentState* experiment_state = (PredictExperimentState*)wrapper->experiment_context[wrapper->experiment_context.size() - 2];
