...
const int STARTING_NUM_NODES = 10;
...
void Solution::init(ProblemType* problem_type) {
	...
	new_scope->num_states = NUM_STATES;
	...
	new_scope->end_score_network = new ScoreNetwork(NUM_STATES);
	...
	for (int a_index = 0; a_index < this->num_actions; a_index++) {
		...
		new_action_node->action_network = new ActionNetwork(NUM_STATES);
