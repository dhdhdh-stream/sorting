// - lower train iters way better
// - lessen impact of any particular set of data

// - simply not bother, and just constantly train
//   - with large epoch size
...
// const int NUM_TRAIN_DATA = 2000;
const int NUM_TRAIN_DATA = 4000;
...
int main(int argc, char* argv[]) {
	...
	for (int epoch_index = 0; epoch_index < 100; epoch_index++) {
		...
		Network* existing_network = new Network(network);

		vector<double> existing_vals(NUM_TRAIN_DATA);
		for (int t_index = 0; t_index < NUM_TRAIN_DATA; t_index++) {
			existing_network->activate(train_data[t_index]);
			existing_vals[t_index] = existing_network->output->acti_vals[0];
		}
