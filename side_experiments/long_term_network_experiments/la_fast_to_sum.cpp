...
int main(int argc, char* argv[]) {
	...
	for (int epoch_index = 0; epoch_index < 100; epoch_index++) {
		vector<vector<double>> train_data(NEW_SAMPLES_PER_EPOCH);
		vector<double> train_targets(NEW_SAMPLES_PER_EPOCH);
		vector<double> train_existing(NEW_SAMPLES_PER_EPOCH);
		for (int t_index = 0; t_index < NEW_SAMPLES_PER_EPOCH; t_index++) {
			...
			train_existing[t_index] = network->output->acti_vals[0];
		}

		int original_weight = 10 + epoch_index;

		for (int c_index = 0; c_index < CYCLES_PER_EPOCH; c_index++) {
			for (int t_index = 0; t_index < NEW_SAMPLES_PER_EPOCH; t_index++) {
				network->activate(train_data[t_index]);

				double adjusted_target = (original_weight * train_existing[t_index] + train_targets[t_index]) / (1.0 + original_weight);
				double error = adjusted_target - network->output->acti_vals[0];
