...
int main(int argc, char* argv[]) {
	...
	uniform_int_distribution<int> base_distribution(0, 9);
	...
	for (int iter_index = 0; iter_index < 1000000; iter_index++) {
		...
		mean = 0.999 * mean + 0.001 * base;
		double curr_variance = (base - mean) * (base - mean);
		variance = 0.999 * variance + 0.001 * curr_variance;
		...
		double curr_end_variance = (base_4 - end_mean) * (base_4 - end_mean);
		end_variance = 0.999 * end_variance + 0.001 * curr_end_variance;
	...
	normal_distribution<double> init_distribution(mean, standard_deviation);
	for (int iter_index = 0; iter_index < 40; iter_index++) {
		cout << iter_index << endl;

		double start = init_distribution(generator);
