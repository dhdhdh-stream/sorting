...
void CommitExperiment::train_existing_backprop(
	...
	if (this->state_iter >= TRAIN_EXISTING_NUM_DATAPOINTS) {
		...
		case NODE_TYPE_ACTION:
			{
				ActionNode* action_node = (ActionNode*)this->node_context;
				this->node_context_next_node = action_node->next_node;
			}
			break;
		case NODE_TYPE_SCOPE:
			{
				ScopeNode* scope_node = (ScopeNode*)this->node_context;
				this->node_context_next_node = scope_node->next_node;
			}
			break;
		case NODE_TYPE_BRANCH:
			{
				BranchNode* branch_node = (BranchNode*)this->node_context;
				if (this->is_branch) {
					this->node_context_next_node = branch_node->branch_next_node;
				} else {
					this->node_context_next_node = branch_node->original_next_node;
				}
			}
			break;
		case NODE_TYPE_OBS:
			{
				ObsNode* obs_node = (ObsNode*)this->node_context;
				this->node_context_next_node = obs_node->next_node;
			}
			break;
		}
