...
int main(int argc, char* argv[]) {
	...
	while (solution->timestamp < EXPLORE_ITERS) {
		...
		int num_meaningful_nodes = last_updated_scope->get_num_meaningful_nodes();
		if (num_meaningful_nodes >= SCOPE_EXCEEDED_NUM_NODES) {
			last_updated_scope->exceeded = true;
		}
		if (num_meaningful_nodes <= SCOPE_RESUME_NUM_NODES) {
