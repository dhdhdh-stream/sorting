				...
				case NODE_TYPE_NOOP:
					{
						...
						for (int h_index = 0; h_index < (int)noop_node->sample_obs.size(); h_index++) {
							noop_node->network->activate(noop_node->sample_obs[h_index]);
							this->new_network->activate(noop_node->sample_obs[h_index]);
							if (this->new_network->output->acti_vals(0) >= noop_node->network->output->acti_vals(0)) {
								existing_sum_vals += noop_node->sample_target_vals[h_index];
								existing_count++;
							}
						}
						...
						for (int h_index = num_new_train; h_index < (int)this->new_obs_histories.size(); h_index++) {
							noop_node->network->activate(this->new_obs_histories[h_index]);
						...
						double average_ratio = (existing_count + new_count)
							/ ((double)noop_node->sample_obs.size()
								+ (double)this->new_obs_histories.size() - num_new_train);
						local_improvement = (new_average - existing_average) * average_ratio;
					}
					break;
				case NODE_TYPE_ACTION:
					{
						...
						double existing_sum_vals = 0.0;
						int existing_count = 0;
						for (int h_index = 0; h_index < (int)action_node->sample_obs.size(); h_index++) {
							action_node->network->activate(action_node->sample_obs[h_index]);
							this->new_network->activate(action_node->sample_obs[h_index]);
							if (this->new_network->output->acti_vals(0) >= action_node->network->output->acti_vals(0)) {
								existing_sum_vals += action_node->sample_target_vals[h_index];
								existing_count++;
							}
						}
						double existing_average = existing_sum_vals / (double)existing_count;
						...
						int new_count = 0;
						for (int h_index = num_new_train; h_index < (int)this->new_obs_histories.size(); h_index++) {
							action_node->network->activate(this->new_obs_histories[h_index]);
							this->new_network->activate(this->new_obs_histories[h_index]);
							if (this->new_network->output->acti_vals[0] >= action_node->network->output->acti_vals[0]) {
								new_sum_vals += this->new_target_val_histories[h_index];
								new_count++;
						...
						double new_average = new_sum_vals / (double)new_count;
						double average_ratio = (existing_count + new_count)
							/ ((double)action_node->sample_obs.size()
								+ (double)this->new_obs_histories.size() - num_new_train);
						local_improvement = (new_average - existing_average) * average_ratio;
					}
					break;
				case NODE_TYPE_SCOPE:
					{
						...
						double existing_sum_vals = 0.0;
						int existing_count = 0;
						for (int h_index = 0; h_index < (int)scope_node->sample_obs.size(); h_index++) {
							scope_node->network->activate(scope_node->sample_obs[h_index]);
							this->new_network->activate(scope_node->sample_obs[h_index]);
							if (this->new_network->output->acti_vals(0) >= scope_node->network->output->acti_vals(0)) {
								existing_sum_vals += scope_node->sample_target_vals[h_index];
								existing_count++;
							}
						}
						double existing_average = existing_sum_vals / (double)existing_count;
						...
						int new_count = 0;
						for (int h_index = num_new_train; h_index < (int)this->new_obs_histories.size(); h_index++) {
							scope_node->network->activate(this->new_obs_histories[h_index]);
							this->new_network->activate(this->new_obs_histories[h_index]);
							if (this->new_network->output->acti_vals[0] >= scope_node->network->output->acti_vals[0]) {
								new_sum_vals += this->new_target_val_histories[h_index];
								new_count++;
						...
						double new_average = new_sum_vals / (double)new_count;
						double average_ratio = (existing_count + new_count)
							/ ((double)scope_node->sample_obs.size()
								+ (double)this->new_obs_histories.size() - num_new_train);
						local_improvement = (new_average - existing_average) * average_ratio;
					}
					break;
				case NODE_TYPE_BRANCH:
					{
						...
						if (this->is_branch) {
							double existing_sum_vals = 0.0;
							int existing_count = 0;
							for (int h_index = 0; h_index < (int)branch_node->branch_sample_obs.size(); h_index++) {
								branch_node->branch_network->activate(branch_node->branch_sample_obs[h_index]);
								this->new_network->activate(branch_node->branch_sample_obs[h_index]);
								if (this->new_network->output->acti_vals(0) >= branch_node->branch_network->output->acti_vals(0)) {
									existing_sum_vals += branch_node->branch_sample_target_vals[h_index];
									existing_count++;
								}
							}
							double existing_average = existing_sum_vals / (double)existing_count;
							...
							int new_count = 0;
							for (int h_index = num_new_train; h_index < (int)this->new_obs_histories.size(); h_index++) {
								branch_node->branch_network->activate(this->new_obs_histories[h_index]);
								this->new_network->activate(this->new_obs_histories[h_index]);
								if (this->new_network->output->acti_vals[0] >= branch_node->branch_network->output->acti_vals[0]) {
									new_sum_vals += this->new_target_val_histories[h_index];
									new_count++;
							...
							double new_average = new_sum_vals / (double)new_count;
							double average_ratio = (existing_count + new_count)
								/ ((double)branch_node->branch_sample_obs.size()
									+ (double)this->new_obs_histories.size() - num_new_train);
							local_improvement = (new_average - existing_average) * average_ratio;
						} else {
							double existing_sum_vals = 0.0;
							int existing_count = 0;
							for (int h_index = 0; h_index < (int)branch_node->original_sample_obs.size(); h_index++) {
								branch_node->original_network->activate(branch_node->original_sample_obs[h_index]);
								this->new_network->activate(branch_node->original_sample_obs[h_index]);
								if (this->new_network->output->acti_vals(0) >= branch_node->original_network->output->acti_vals(0)) {
									existing_sum_vals += branch_node->original_sample_target_vals[h_index];
									existing_count++;
								}
							}
							double existing_average = existing_sum_vals / (double)existing_count;
							...
							int new_count = 0;
							for (int h_index = num_new_train; h_index < (int)this->new_obs_histories.size(); h_index++) {
								branch_node->original_network->activate(this->new_obs_histories[h_index]);
								this->new_network->activate(this->new_obs_histories[h_index]);
								if (this->new_network->output->acti_vals[0] >= branch_node->original_network->output->acti_vals[0]) {
									new_sum_vals += this->new_target_val_histories[h_index];
									new_count++;
							...
							double new_average = new_sum_vals / (double)new_count;
							double average_ratio = (existing_count + new_count)
								/ ((double)branch_node->original_sample_obs.size()
									+ (double)this->new_obs_histories.size() - num_new_train);
							local_improvement = (new_average - existing_average) * average_ratio;
						}
					}
