			...
			case NODE_TYPE_NOOP:
				{
					...
					noop_node->network->activate(obs);
					history->existing_predicted.push_back(noop_node->network->output->acti_vals[0]);
				}
				break;
			case NODE_TYPE_ACTION:
				{
					ActionNode* action_node = (ActionNode*)this->node_context;
					action_node->network->activate(obs);
					history->existing_predicted.push_back(action_node->network->output->acti_vals[0]);
				}
				break;
			case NODE_TYPE_SCOPE:
				{
					ScopeNode* scope_node = (ScopeNode*)this->node_context;
					scope_node->network->activate(obs);
					history->existing_predicted.push_back(scope_node->network->output->acti_vals[0]);
				}
				break;
