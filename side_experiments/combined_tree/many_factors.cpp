...
int main(int argc, char* argv[]) {
	...
	DecisionTree* sum_tree = new DecisionTree();
	...
	for (int iter_index = 0; iter_index < 100000; iter_index++) {
		...
		auto starting_history_size = sum_tree->improvement_history.size();

		sum_tree->backprop(inputs,
						   target_val);

		if (starting_history_size != sum_tree->improvement_history.size()) {
			ofstream output_file;
			output_file.open("saves/main.txt");

			sum_tree->save(output_file);
