...
EvalNetwork::EvalNetwork(int num_obs,
						 int num_actions,
						 int num_state) {
	...
	this->action_input = new Layer(num_actions);
...
EvalNetwork::EvalNetwork(ifstream& input_file) {
...
void EvalNetwork::activate(vector<double>& obs_vals,
	...
	for (int i_index = 0; i_index < (int)this->action_input->acti_vals.size(); i_index++) {
		history->action_input_history[i_index] = this->action_input->acti_vals[i_index];
	}
	...
	for (int i_index = 0; i_index < (int)this->state_input->acti_vals.size(); i_index++) {
		history->state_input_history[i_index] = this->state_input->acti_vals[i_index];
	}
...
void EvalNetwork::backprop(double error,
	...
	for (int i_index = 0; i_index < (int)this->action_input->acti_vals.size(); i_index++) {
		this->action_input->acti_vals[i_index] = history->action_input_history[i_index];
	}
	for (int i_index = 0; i_index < (int)this->state_input->acti_vals.size(); i_index++) {
		this->state_input->acti_vals[i_index] = history->state_input_history[i_index];
	}
	for (int n_index = 0; n_index < (int)this->hidden_1->acti_vals.size(); n_index++) {
...
void EvalNetwork::save(ofstream& output_file) {
