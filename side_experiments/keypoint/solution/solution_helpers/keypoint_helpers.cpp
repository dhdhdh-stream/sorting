	...
	if (scope_history->scope == explore_node->parent) {
		...
		if (keypoint_it != scope_history->node_histories.end()) {
			...
			if (has_dependencies) {
				...
				target_val_histories.push_back(keypoint_it->second->obs_history[explore_obs_index]);
			} else {
				num_miss++;
			}
		}
...
void keypoint_experiment(vector<ScopeHistory*>& scope_histories) {
	...
	ObsNodeHistory* explore_node_history = possible_nodes[node_distribution(generator)];
	...
	double new_availibility = (double)num_hit / (double)num_total;

	bool higher_availability = false;
	if (explore_node->keypoints[explore_obs_index] == NULL) {
		higher_availability = true;
	} else {
		if (new_availibility > explore_node->keypoints[explore_obs_index]->availibility) {
			higher_availability = true;
		}
	}
	...
	if (higher_availability) {
		...
		double obs_misguess = sqrt(solution->obs_variance[explore_obs_index]);
		if (new_average_misguess < KEYPOINT_MAX_MISGUESS_RATIO * obs_misguess) {
			...
			new_keypoint->availibility = new_availibility;
