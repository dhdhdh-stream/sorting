...
bool SignalEvalExperiment::split_helper(vector<bool>& new_match_input_is_pre,
	...
	int num_positive_seeds = POSITIVE_SEED_RATIO * (double)this->pre_obs.size();
	...
	int num_negative_seeds = NEGATIVE_SEED_RATIO * (double)this->explore_pre_obs.size();
			...
			if (is_positive) {
				...
				for (int i_index = 0; i_index < (int)new_match_input_is_pre.size(); i_index++) {
					if (new_match_input_is_pre[i_index]) {
						inputs[i_index] = this->pre_obs[h_index][
							new_match_input_indexes[i_index]][new_match_input_obs_indexes[i_index]];
					} else {
						inputs[i_index] = this->post_obs[h_index][
							new_match_input_indexes[i_index]][new_match_input_obs_indexes[i_index]];
					}
				}
			} else {
