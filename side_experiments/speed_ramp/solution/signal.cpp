...
void Signal::calc(vector<vector<double>>& pre_obs_histories,
	...
	for (int i_index = 0; i_index < (int)this->match_input_is_pre.size(); i_index++) {
		if (this->match_input_is_pre[i_index]) {
			input_vals[i_index] = pre_obs_histories[
				this->match_input_indexes[i_index]][this->match_input_obs_indexes[i_index]];
		} else {
	...
	if (this->match_network->output->acti_vals[0] >= MATCH_WEIGHT) {
		...
		is_match = true;
		val = this->score_network->output->acti_vals[0];
	} else {
		is_match = false;
	}
}
