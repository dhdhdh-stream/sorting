...
void Solution::init(ProblemType* problem_type) {
	...
	Scope* l0_scope = new Scope();
	l0_scope->is_outer = false;
	l0_scope->id = this->scopes.size();
	l0_scope->is_leaf = false;
	l0_scope->node_counter = 0;
	this->scopes.push_back(l0_scope);

	Scope* l0_front_scope = new Scope();
	l0_front_scope->is_outer = false;
	l0_front_scope->id = this->scopes.size();
	l0_front_scope->is_leaf = true;
	l0_front_scope->node_counter = 0;
	this->scopes.push_back(l0_front_scope);

	Scope* l0_back_scope = new Scope();
	l0_back_scope->is_outer = false;
	l0_back_scope->id = this->scopes.size();
	l0_back_scope->is_leaf = true;
	l0_back_scope->node_counter = 0;
	this->scopes.push_back(l0_back_scope);

	Scope* l1_scope = new Scope();
	l1_scope->is_outer = false;
	l1_scope->id = this->scopes.size();
	l1_scope->is_leaf = false;
	l1_scope->node_counter = 0;
	this->scopes.push_back(l1_scope);

	Scope* l1_front_scope = new Scope();
	l1_front_scope->is_outer = false;
	l1_front_scope->id = this->scopes.size();
	l1_front_scope->is_leaf = true;
	l1_front_scope->node_counter = 0;
	this->scopes.push_back(l1_front_scope);

	Scope* l1_back_scope = new Scope();
	l1_back_scope->is_outer = false;
	l1_back_scope->id = this->scopes.size();
	l1_back_scope->is_leaf = true;
	l1_back_scope->node_counter = 0;
	this->scopes.push_back(l1_back_scope);

	Scope* l2_scope = new Scope();
	l2_scope->is_outer = false;
	l2_scope->id = this->scopes.size();
	l2_scope->is_leaf = true;
	l2_scope->node_counter = 0;
	this->scopes.push_back(l2_scope);

	{
		StartNode* start_node = new StartNode();
		start_node->parent = l2_scope;
		start_node->id = l2_scope->node_counter;
		l2_scope->node_counter++;
		l2_scope->nodes[start_node->id] = start_node;

		ObsNode* end_node = new ObsNode();
		end_node->parent = l2_scope;
		end_node->id = l2_scope->node_counter;
		l2_scope->node_counter++;
		l2_scope->nodes[end_node->id] = end_node;
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l1_back_scope;
		start_node->id = l1_back_scope->node_counter;
		l1_back_scope->node_counter++;
		l1_back_scope->nodes[start_node->id] = start_node;

		ObsNode* end_node = new ObsNode();
		end_node->parent = l1_back_scope;
		end_node->id = l1_back_scope->node_counter;
		l1_back_scope->node_counter++;
		l1_back_scope->nodes[end_node->id] = end_node;
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l1_front_scope;
		start_node->id = l1_front_scope->node_counter;
		l1_front_scope->node_counter++;
		l1_front_scope->nodes[start_node->id] = start_node;

		ObsNode* end_node = new ObsNode();
		end_node->parent = l1_front_scope;
		end_node->id = l1_front_scope->node_counter;
		l1_front_scope->node_counter++;
		l1_front_scope->nodes[end_node->id] = end_node;
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l1_scope;
		start_node->id = l1_scope->node_counter;
		...
		ObsNode* end_node = new ObsNode();
		end_node->parent = l1_scope;
		end_node->id = l1_scope->node_counter;
		l1_scope->node_counter++;
		l1_scope->nodes[end_node->id] = end_node;

		start_node->next_node_id = scope_nodes[0]->id;
		start_node->next_node = scope_nodes[0];
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l0_back_scope;
		start_node->id = l0_back_scope->node_counter;
		l0_back_scope->node_counter++;
		l0_back_scope->nodes[start_node->id] = start_node;

		ObsNode* end_node = new ObsNode();
		end_node->parent = l0_back_scope;
		end_node->id = l0_back_scope->node_counter;
		l0_back_scope->node_counter++;
		l0_back_scope->nodes[end_node->id] = end_node;
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l0_front_scope;
		start_node->id = l0_front_scope->node_counter;
		l0_front_scope->node_counter++;
		l0_front_scope->nodes[start_node->id] = start_node;

		ObsNode* end_node = new ObsNode();
		end_node->parent = l0_front_scope;
		end_node->id = l0_front_scope->node_counter;
		l0_front_scope->node_counter++;
		l0_front_scope->nodes[end_node->id] = end_node;
	...
	{
		StartNode* start_node = new StartNode();
		start_node->parent = l0_scope;
		start_node->id = l0_scope->node_counter;
		...
		ObsNode* end_node = new ObsNode();
		end_node->parent = l0_scope;
		end_node->id = l0_scope->node_counter;
		l0_scope->node_counter++;
		l0_scope->nodes[end_node->id] = end_node;
