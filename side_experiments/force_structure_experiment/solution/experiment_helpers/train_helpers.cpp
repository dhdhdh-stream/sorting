...
void Experiment::train_and_eval_helper(double& best_val_average,
									   Network*& best_network,
									   bool& best_is_binarize) {
	int num_train_samples = (1.0 - VALIDATION_RATIO) * (int)this->new_obs_histories.size();

	Network* curr_new_network = new Network(this->new_obs_histories[0].size(),
											NETWORK_SIZE_SMALL);
	uniform_int_distribution<int> new_distribution(0, num_train_samples-1);
	for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
		int index = new_distribution(generator);

		curr_new_network->activate(this->new_obs_histories[index]);

		double error = this->new_true_histories[index] - curr_new_network->output->acti_vals[0];
