...
void train_decision_on_state_helper(ProblemType* problem_type,
	...
	Network* network_1;
	{
		...
		for (int i_index = 0; i_index < NUM_TRAIN_SAMPLES; i_index++) {
			...
			vector<double> input{state.data(), state.data() + state.size()};

			obs_histories.push_back(input);
	...
	for (int iter_index = 0; iter_index < NUM_MEASURE_SAMPLES; iter_index++) {
		...
		network_1->activate(input);
		network_2->activate(input);
	...
	double average_score = sum_scores / NUM_MEASURE_SAMPLES;
	cout << "train decision on state: " << average_score << endl;
