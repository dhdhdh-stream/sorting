#include "solution_helpers.h"

#include <iostream>

#include "action_network.h"
#include "constants.h"
#include "globals.h"
#include "network.h"
#include "obs_network.h"
#include "problem.h"
#include "score_network.h"
#include "solution.h"

using namespace std;

#if defined(MDEBUG) && MDEBUG
const int LOW_NUM_SAMPLES = 20;
const int LOW_TRAIN_ITERS = 30;
#else
const int LOW_NUM_SAMPLES = 100;
const int LOW_TRAIN_ITERS = 100000;
#endif /* MDEBUG */

void compare(ProblemType* problem_type,
			 vector<int>& actions) {
	vector<vector<double>> obs_histories;
	vector<double> target_val_histories;
	for (int i_index = 0; i_index < NUM_TRAIN_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();
		obs_histories.push_back(obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		target_val_histories.push_back(target_val);

		delete problem;
	}

	Network* network = new Network(obs_histories[0].size());
	double hidden_1_average_max_update = 0.0;
	double hidden_2_average_max_update = 0.0;
	double hidden_3_average_max_update = 0.0;
	double output_average_max_update = 0.0;

	uniform_int_distribution<int> new_train_distribution(0, obs_histories.size()-1);
	for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
		int rand_index = new_train_distribution(generator);

		network->activate(obs_histories[rand_index]);

		double error = target_val_histories[rand_index] - network->output->acti_vals[0];

		network->init_backprop(error,
							   hidden_1_average_max_update,
							   hidden_2_average_max_update,
							   hidden_3_average_max_update,
							   output_average_max_update);
	}

	double sum_misguess = 0.0;
	for (int h_index = 0; h_index < (int)obs_histories.size(); h_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();

		network->activate(obs_histories[h_index]);
		double predicted = network->output->acti_vals(0);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		sum_misguess += (target_val - predicted) * (target_val - predicted);

		delete problem;
	}
	double misguess_average = sum_misguess / (double)obs_histories.size();
	cout << "compare: " << misguess_average << endl;

	delete network;
}

void low_samples(ProblemType* problem_type,
				 vector<int>& actions) {
	vector<vector<double>> obs_histories;
	vector<double> target_val_histories;
	for (int i_index = 0; i_index < LOW_NUM_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();
		obs_histories.push_back(obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		target_val_histories.push_back(target_val);

		delete problem;
	}

	Network* network = new Network(obs_histories[0].size());
	double hidden_1_average_max_update = 0.0;
	double hidden_2_average_max_update = 0.0;
	double hidden_3_average_max_update = 0.0;
	double output_average_max_update = 0.0;

	uniform_int_distribution<int> new_train_distribution(0, obs_histories.size()-1);
	for (int iter_index = 0; iter_index < LOW_TRAIN_ITERS; iter_index++) {
		int rand_index = new_train_distribution(generator);

		network->activate(obs_histories[rand_index]);

		double error = target_val_histories[rand_index] - network->output->acti_vals[0];

		network->init_backprop(error,
							   hidden_1_average_max_update,
							   hidden_2_average_max_update,
							   hidden_3_average_max_update,
							   output_average_max_update);
	}

	double sum_misguess = 0.0;
	for (int h_index = 0; h_index < (int)obs_histories.size(); h_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();

		network->activate(obs_histories[h_index]);
		double predicted = network->output->acti_vals(0);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		sum_misguess += (target_val - predicted) * (target_val - predicted);

		delete problem;
	}
	double misguess_average = sum_misguess / (double)obs_histories.size();
	cout << "Low: " << misguess_average << endl;

	delete network;
}

void low_samples_w_average(ProblemType* problem_type,
						   vector<int>& actions) {
	vector<vector<double>> obs_histories;
	vector<double> target_val_histories;
	for (int i_index = 0; i_index < LOW_NUM_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();
		obs_histories.push_back(obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		target_val_histories.push_back(target_val);

		delete problem;
	}

	vector<vector<double>> average_obs_histories;
	double sum_vals = 0.0;
	for (int i_index = 0; i_index < NUM_TRAIN_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();
		average_obs_histories.push_back(obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		sum_vals += target_val;

		delete problem;
	}
	double val_average = sum_vals / NUM_TRAIN_SAMPLES;

	Network* network = new Network(obs_histories[0].size());
	double hidden_1_average_max_update = 0.0;
	double hidden_2_average_max_update = 0.0;
	double hidden_3_average_max_update = 0.0;
	double output_average_max_update = 0.0;

	uniform_int_distribution<int> is_low_distribution(0, 1);
	uniform_int_distribution<int> low_sample_distribution(0, obs_histories.size()-1);
	uniform_int_distribution<int> average_sample_distribution(0, average_obs_histories.size()-1);
	for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
		if (is_low_distribution(generator) == 0) {
			int rand_index = low_sample_distribution(generator);

			network->activate(obs_histories[rand_index]);

			double error = target_val_histories[rand_index] - network->output->acti_vals[0];

			network->init_backprop(error,
								   hidden_1_average_max_update,
								   hidden_2_average_max_update,
								   hidden_3_average_max_update,
								   output_average_max_update);
		} else {
			int rand_index = average_sample_distribution(generator);

			network->activate(average_obs_histories[rand_index]);

			double error = val_average - network->output->acti_vals[0];

			network->init_backprop(error,
								   hidden_1_average_max_update,
								   hidden_2_average_max_update,
								   hidden_3_average_max_update,
								   output_average_max_update);
		}
	}

	double sum_misguess = 0.0;
	for (int h_index = 0; h_index < (int)obs_histories.size(); h_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();

		network->activate(obs_histories[h_index]);
		double predicted = network->output->acti_vals(0);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		sum_misguess += (target_val - predicted) * (target_val - predicted);

		delete problem;
	}
	double misguess_average = sum_misguess / (double)obs_histories.size();
	cout << "Low w/ average: " << misguess_average << endl;

	delete network;
}

void low_samples_w_predict(ProblemType* problem_type,
						   vector<int>& actions,
						   Solution* solution) {
	vector<vector<double>> obs_histories;
	vector<double> target_val_histories;
	for (int i_index = 0; i_index < LOW_NUM_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();
		obs_histories.push_back(obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		target_val_histories.push_back(target_val);

		delete problem;
	}

	vector<vector<double>> predict_obs_histories;
	vector<double> predict_target_val_histories;
	for (int i_index = 0; i_index < NUM_TRAIN_SAMPLES; i_index++) {
		Problem* problem = problem_type->get_problem();

		Eigen::VectorXf state;
		state.resize(NUM_STATES);
		state.setConstant(0.0);

		vector<double> obs = problem->get_observations();

		predict_obs_histories.push_back(obs);

		solution->obs_network->activate(state,
										obs);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			solution->action_networks[actions[a_index]]->activate(state);
		}

		solution->score_network->activate(state);
		double predicted = solution->score_network->output->acti_vals(0);
		predict_target_val_histories.push_back(predicted);

		delete problem;
	}

	Network* network = new Network(obs_histories[0].size());
	double hidden_1_average_max_update = 0.0;
	double hidden_2_average_max_update = 0.0;
	double hidden_3_average_max_update = 0.0;
	double output_average_max_update = 0.0;

	uniform_int_distribution<int> is_low_distribution(0, 1);
	uniform_int_distribution<int> low_sample_distribution(0, obs_histories.size()-1);
	uniform_int_distribution<int> predict_sample_distribution(0, predict_obs_histories.size()-1);
	for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
		if (is_low_distribution(generator) == 0) {
			int rand_index = low_sample_distribution(generator);

			network->activate(obs_histories[rand_index]);

			double error = target_val_histories[rand_index] - network->output->acti_vals[0];

			network->init_backprop(error,
								   hidden_1_average_max_update,
								   hidden_2_average_max_update,
								   hidden_3_average_max_update,
								   output_average_max_update);
		} else {
			int rand_index = predict_sample_distribution(generator);

			network->activate(predict_obs_histories[rand_index]);

			double error = predict_target_val_histories[rand_index] - network->output->acti_vals[0];

			network->init_backprop(error,
								   hidden_1_average_max_update,
								   hidden_2_average_max_update,
								   hidden_3_average_max_update,
								   output_average_max_update);
		}
	}

	double sum_misguess = 0.0;
	for (int h_index = 0; h_index < (int)obs_histories.size(); h_index++) {
		Problem* problem = problem_type->get_problem();

		vector<double> obs = problem->get_observations();

		network->activate(obs_histories[h_index]);
		double predicted = network->output->acti_vals(0);

		for (int a_index = 0; a_index < (int)actions.size(); a_index++) {
			problem->perform_action(actions[a_index]);
		}

		double target_val = problem->score_result();
		sum_misguess += (target_val - predicted) * (target_val - predicted);

		delete problem;
	}
	double misguess_average = sum_misguess / (double)obs_histories.size();
	cout << "Low w/ predict: " << misguess_average << endl;

	delete network;
}
