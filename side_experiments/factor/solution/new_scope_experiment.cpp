...
NewScopeExperiment::NewScopeExperiment(Scope* scope_context,
	...
	if (this->new_scope != NULL) {
		...
		AbstractNode* random_start_node = node_context->next_node;
...
NewScopeExperiment::~NewScopeExperiment() {
	...
	for (int o_index = 0; o_index < (int)this->successful_obs_nodes.size(); o_index++) {
		delete this->successful_obs_nodes[o_index];
	}
...
void NewScopeExperiment::decrement(ObsNode* experiment_node) {
	bool is_test;
	int location_index;
	...
	if (is_test) {
		...
		ObsNode* new_obs_node = this->successful_obs_nodes[location_index];
		this->successful_obs_nodes.erase(this->successful_obs_nodes.begin() + location_index);
		delete new_obs_node;
	}
