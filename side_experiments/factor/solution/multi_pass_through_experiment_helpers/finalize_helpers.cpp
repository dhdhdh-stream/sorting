...
void MultiPassThroughExperiment::finalize(Solution* duplicate) {
	if (this->result == EXPERIMENT_RESULT_SUCCESS) {
		...
		for (int s_index = 0; s_index < (int)this->step_types.size(); s_index++) {
			if (this->step_types[s_index] == STEP_TYPE_ACTION) {
				...
				new_scope_node->scope = duplicate->scopes[this->scopes[s_index]->id];
		...
		if (this->exit_next_node == NULL) {
			...
			exit_node = duplicate_local_scope->nodes[this->exit_next_node->id];
		}
