...
void CommitExperiment::add() {
	...
	this->node_context->next_node_id = start_node_id;
	this->node_context->next_node = start_node;

	start_node->ancestor_ids.push_back(this->node_context->id);
	...
	for (int f_index = 0; f_index < (int)this->commit_new_factor_ids.size(); f_index++) {
		ObsNode* obs_node = (ObsNode*)this->scope_context->nodes[this->commit_new_factor_ids[f_index].first];
