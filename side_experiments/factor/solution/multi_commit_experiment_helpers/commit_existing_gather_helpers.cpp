...
void MultiCommitExperiment::commit_existing_gather_activate(
...
void MultiCommitExperiment::commit_existing_gather_backprop(
		MultiCommitExperimentHistory* history) {
	if (history->is_active) {
		this->state_iter++;
		if (this->state_iter >= GATHER_ITERS) {
			...
			this->state = MULTI_COMMIT_EXPERIMENT_STATE_COMMIT_TRAIN_EXISTING;
			this->state_iter = 0;
		}
	}
}
