...
void MultiCommitExperiment::commit_train_existing_activate(
...
void MultiCommitExperiment::commit_train_existing_backprop(
		double target_val,
		RunHelper& run_helper,
		MultiCommitExperimentHistory* history) {
	if (history->is_active) {
		...
		if (this->state_iter >= TRAIN_EXISTING_NUM_DATAPOINTS) {
			...
			if (new_t_score > 2.326) {
				...
				for (int i_index = (int)this->commit_existing_inputs.size()-1; i_index >= 0; i_index--) {
					vector<pair<pair<vector<Scope*>,vector<int>>,pair<int,int>>> remove_inputs = this->commit_existing_inputs;
			...
			this->state = MULTI_COMMIT_EXPERIMENT_STATE_COMMIT_NEW_GATHER;
			this->state_iter = 0;
		}
	}
}
