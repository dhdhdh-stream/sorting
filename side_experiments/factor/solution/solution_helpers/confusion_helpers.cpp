...
void confusion_gather_nodes_seen_helper(ScopeHistory* scope_history,
...
void create_confusion(ScopeHistory* scope_history) {
	map<pair<AbstractNode*,bool>,int> nodes_seen;
	confusion_gather_nodes_seen_helper(scope_history,
									   nodes_seen);
	...
	if (nodes_seen.size() > 0) {
		...
		Confusion* confusion = new Confusion(explore_node->parent,
											 explore_node,
											 explore_is_branch);
		explore_node->confusion = confusion;
	}
}
