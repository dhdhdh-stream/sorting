...
void NewScopeExperiment::add() {
	...
	for (int s_index = 0; s_index < (int)this->successful_location_starts.size(); s_index++) {
		ObsNode* new_obs_node = this->successful_obs_nodes[s_index];
		...
		new_scope_node->next_node_id = new_obs_node->id;
		new_scope_node->next_node = new_obs_node;

		if (new_obs_node->next_node == NULL) {
			...
			new_obs_node->next_node_id = new_local_ending_node->id;
			new_obs_node->next_node = new_local_ending_node;

			new_local_ending_node->ancestor_ids.push_back(new_obs_node->id);
		} else {
			new_obs_node->next_node->ancestor_ids.push_back(new_obs_node->id);
		}

		for (int a_index = 0; a_index < (int)this->successful_location_starts[s_index]->next_node->ancestor_ids.size(); a_index++) {
			if (this->successful_location_starts[s_index]->next_node->ancestor_ids[a_index] == this->successful_location_starts[s_index]->id) {
				this->successful_location_starts[s_index]->next_node->ancestor_ids.erase(
					this->successful_location_starts[s_index]->next_node->ancestor_ids.begin() + a_index);
				break;
			}
		}

		this->successful_location_starts[s_index]->next_node_id = new_scope_node->id;
		this->successful_location_starts[s_index]->next_node = new_scope_node;

		new_scope_node->ancestor_ids.push_back(this->successful_location_starts[s_index]->id);
	}
	this->successful_obs_nodes.clear();
