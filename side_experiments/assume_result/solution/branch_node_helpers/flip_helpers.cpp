...
void BranchNode::flip_gather_activate(AbstractNode*& curr_node,
									  Problem* problem,
	...
	branch_node_indexes.push_back(run_helper.num_actions);
...
void BranchNode::flip_activate(AbstractNode*& curr_node,
	...
	if (run_helper.num_actions == target_branch_node_index) {
		if (is_branch) {
			curr_node = this->original_next_node;
		} else {
			curr_node = this->branch_next_node;
