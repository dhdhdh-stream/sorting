...
Solution::Solution(Solution* original) {
	...
	this->last_experiment_node = NULL;
}
...
void Solution::load(string path,
	...
	if (subproblem_id == -1) {
		...
		this->subproblem_exit_node = scope->nodes[stoi(exit_node_id_line)];
	}
	this->last_experiment_node = NULL;
...
void Solution::create_subproblem() {
	...
	if (this->subproblem == NULL
		...
		Sample sample(sample_distribution(generator));
		...
		this->subproblem_starting_node = this->last_experiment_node;
