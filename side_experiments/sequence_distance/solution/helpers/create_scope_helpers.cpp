			...
			case NODE_TYPE_ACTION:
				{
					...
					if (next_mapping.first) {
						...
						new_potential_scope_node->original_nodes.push_back(action_node);
			...
			case NODE_TYPE_SCOPE:
				{
					...
					if (next_mapping.first) {
						...
						new_potential_scope_node->original_nodes.push_back(scope_node);
					...
					if (matches_context) {
						if (branch_node->branch_is_pass_through) {
							...
							if (original_mapping.first && branch_mapping.first) {
								...
								new_potential_scope_node->original_nodes.push_back(branch_node);
...
int start_new_state_helper(map<pair<bool,int>, int>& state_mappings,
						   ...
						   Scope* new_scope,
						   PotentialScopeNode* new_potential_scope_node,
						   vector<pair<pair<bool,int>, pair<bool,int>>>& potential_to_highest_mapping) {
	...
	if (it != state_mappings.end()) {
		...
		if (potential_to_final_states[potential_index] == -1) {
			if (is_local || input_non_empty_distribution(generator) == 0) {
				...
				new_potential_scope_node->original_states.push_back(
					potential_to_highest_mapping[potential_index]);

				if (possible_input_scope_depths.size() > 0 && input_type_distribution(generator) == 0) {
					// state
...
PotentialScopeNode* create_scope(vector<ContextLayer>& context,
	...
	for (int c_index = 0; c_index < (int)start_scope_context.size(); c_index++) {
		new_potential_scope_node->starting_original_scope_context.push_back(start_scope_context[c_index]->id);
	}
	for (int c_index = 0; c_index < (int)start_node_context.size(); c_index++) {
		new_potential_scope_node->starting_original_node_context.push_back(start_node_context[c_index]->id);
	}
	for (int c_index = 0; c_index < (int)end_scope_context.size(); c_index++) {
		new_potential_scope_node->ending_original_scope_context.push_back(end_scope_context[c_index]->id);
	}
	for (int c_index = 0; c_index < (int)end_node_context.size(); c_index++) {
		new_potential_scope_node->ending_original_node_context.push_back(end_node_context[c_index]->id);
	}
	...
	for (int l_index = (int)start_new_node_reverse_mappings.size()-1; l_index >= 0; l_index--) {
			...
			if (node_it->second->type == NODE_TYPE_ACTION) {
				...
				for (int n_index = 0; n_index < (int)original_action_node->state_is_local.size(); n_index++) {
					...
					if (state_it == start_potential_new_state_mappings[l_index].end()) {
						...
						potential_to_highest_mapping.push_back({{true, l_index}, {original_action_node->state_is_local[n_index], original_action_node->state_indexes[n_index]}});
					}
				}
				...
				for (int i_index = 0; i_index < (int)original_scope_node->input_types.size(); i_index++) {
					if (original_scope_node->input_types[i_index] == INPUT_TYPE_STATE) {
						...
						if (state_it == start_potential_new_state_mappings[l_index].end()) {
							...
							potential_new_state_counter++;
							potential_to_highest_mapping.push_back({{true, l_index}, {original_scope_node->input_outer_is_local[i_index], original_scope_node->input_outer_indexes[i_index]}});
						}
					}
				}
				for (int o_index = 0; o_index < (int)original_scope_node->output_inner_indexes.size(); o_index++) {
					...
					if (state_it == start_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{true, l_index}, {original_scope_node->output_outer_is_local[o_index], original_scope_node->output_outer_indexes[o_index]}});
					}
				}
			} else {
				...
				for (int s_index = 0; s_index < (int)original_branch_node->decision_state_is_local.size(); s_index++) {
					...
					if (state_it == start_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{true, l_index}, {original_branch_node->decision_state_is_local[s_index], original_branch_node->decision_state_indexes[s_index]}});
					}
				}
			}
		}
		...
		if (l_index != 0) {
			...
			for (int o_index = 0; o_index < (int)outer_scope_node->output_inner_indexes.size(); o_index++) {
				...
				if (it != start_potential_new_state_mappings[l_index].end()) {
					...
					potential_to_highest_mapping[it->second] = {{true, l_index-1}, {outer_scope_node->output_outer_is_local[o_index], outer_scope_node->output_outer_indexes[o_index]}};
				}
			}
		}
	}
			...
			if (node_it->second->type == NODE_TYPE_ACTION) {
				...
				for (int n_index = 0; n_index < (int)original_action_node->state_is_local.size(); n_index++) {
					...
					if (state_it == end_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{false, l_index}, {original_action_node->state_is_local[n_index], original_action_node->state_indexes[n_index]}});
					}
				}
			} else if (node_it->second->type == NODE_TYPE_SCOPE) {
				ScopeNode* original_scope_node = (ScopeNode*)node_it->second;
				for (int i_index = 0; i_index < (int)original_scope_node->input_types.size(); i_index++) {
					...
					if (state_it == end_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{false, l_index}, {original_scope_node->input_outer_is_local[i_index], original_scope_node->input_outer_indexes[i_index]}});
					}
				}
				for (int o_index = 0; o_index < (int)original_scope_node->output_inner_indexes.size(); o_index++) {
					map<pair<bool,int>, int>::iterator state_it = end_potential_new_state_mappings[l_index]
					...
					if (state_it == end_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{false, l_index}, {original_scope_node->output_outer_is_local[o_index], original_scope_node->output_outer_indexes[o_index]}});
					}
				}
			} else {
				BranchNode* original_branch_node = (BranchNode*)node_it->second;
				for (int s_index = 0; s_index < (int)original_branch_node->decision_state_is_local.size(); s_index++) {
					...
					if (state_it == end_potential_new_state_mappings[l_index].end()) {
						...
						potential_new_state_counter++;
						potential_to_highest_mapping.push_back({{false, l_index}, {original_branch_node->decision_state_is_local[s_index], original_branch_node->decision_state_indexes[s_index]}});
					}
				}
			}
		}
