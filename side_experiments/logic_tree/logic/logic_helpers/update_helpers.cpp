...
const int TARGET_NUM_IMPROVEMENTS = 10;
...
void update(AbstractLogicNode* node,
	...
	for (int h_index = NUM_TRAIN_SAMPLES; h_index < NUM_TOTAL_SAMPLES; h_index++) {
		existing_sum_misguess += (node->target_val_histories[h_index] - node->existing_predicted_histories[h_index])
			* (node->target_val_histories[h_index] - node->existing_predicted_histories[h_index]);
	}
	...
	while (true) {
		...
		double max_num_samples = MAX_RATIO * (double)NUM_TRAIN_SAMPLES;
		double min_num_samples = MIN_RATIO * (double)NUM_TRAIN_SAMPLES;
		...
		for (int iter_index = 0; iter_index < TRAIN_ITERS; iter_index++) {
			...
			potential_eval_network->activate(match_obs[index]);

			double error = match_target_vals[index] - potential_eval_network->output->acti_vals[0];

			potential_eval_network->backprop(error);
		}

		double potential_sum_misguess = 0.0;
		for (int h_index = NUM_TRAIN_SAMPLES; h_index < NUM_TOTAL_SAMPLES; h_index++) {
			bool is_match = is_match_helper(node->obs_histories[h_index],
											potential_obs_index,
											potential_rel_obs_index,
			...
			if (is_match) {
				potential_eval_network->activate(node->obs_histories[h_index]);

				potential_sum_misguess += (node->target_val_histories[h_index] - potential_eval_network->output->acti_vals[0])
					* (node->target_val_histories[h_index] - potential_eval_network->output->acti_vals[0]);
			} else {
				potential_sum_misguess += (node->target_val_histories[h_index] - node->existing_predicted_histories[h_index])
					* (node->target_val_histories[h_index] - node->existing_predicted_histories[h_index]);
			}
		}

		if (potential_sum_misguess < existing_sum_misguess) {
			double improvement = existing_sum_misguess - potential_sum_misguess;
			if (improvement > best_improvement) {
				best_obs_index = potential_obs_index;
				best_rel_obs_index = potential_rel_obs_index;
				best_split_type = potential_split_type;
				best_split_target = potential_split_target;
				best_split_range = potential_split_range;
			...
			improvement_iter++;
			if (improvement_iter >= TARGET_NUM_IMPROVEMENTS) {
				break;
			}
		} else {
			delete potential_eval_network;
	...
	double new_default_sum_misguess = 0.0;
	for (int h_index = NUM_TRAIN_SAMPLES; h_index < NUM_TOTAL_SAMPLES; h_index++) {
		new_default_network->activate(node->obs_histories[h_index]);

		new_default_sum_misguess += (node->target_val_histories[h_index] - new_default_network->output->acti_vals[0])
			* (node->target_val_histories[h_index] - new_default_network->output->acti_vals[0]);
	}
	double new_default_improvement = existing_sum_misguess - new_default_sum_misguess;
	...
	if (new_default_improvement >= best_improvement) {
		...
		new_eval_node->id = wrapper->logic_tree->node_counter;
		wrapper->logic_tree->node_counter++;
		wrapper->logic_tree->nodes[new_eval_node->id] = new_eval_node;

		new_eval_node->network = new_default_network;

		if (parent == NULL) {
			wrapper->logic_tree->root = new_eval_node;
		} else {
			if (is_branch) {
				parent->branch_node_id = new_eval_node->id;
				parent->branch_node = new_eval_node;
			} else {
				parent->original_node_id = new_eval_node->id;
				parent->original_node = new_eval_node;
			}
		}
	} else {
		...
		new_split_node->id = wrapper->logic_tree->node_counter;
		wrapper->logic_tree->node_counter++;
		wrapper->logic_tree->nodes[new_split_node->id] = new_split_node;
		...
		if (parent == NULL) {
			wrapper->logic_tree->root = new_split_node;
		} else {
			if (is_branch) {
	...
	node->existing_predicted_histories.clear();
}
