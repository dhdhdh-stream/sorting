...
void ChaseExperiment::explore_step(vector<double>& obs,
								   int& action,
								   bool& is_next,
								   bool& fetch_action,
...
void ChaseExperiment::explore_set_action(int action,
										 SolutionWrapper* wrapper) {
	ChaseExperimentState* experiment_state = (ChaseExperimentState*)wrapper->experiment_context.back();
...
void ChaseExperiment::explore_exit_step(SolutionWrapper* wrapper) {
...
void ChaseExperiment::explore_backprop(double target_val,
									   SolutionWrapper* wrapper) {
	ChaseExperimentHistory* history = (ChaseExperimentHistory*)wrapper->experiment_history;
	...
	if (history->existing_predicted_trues.size() != 0) {
		...
		if (wrapper->curr_solution->obs_histories.size() < EXPLORE_MAX_SAMPLES) {
			wrapper->curr_solution->obs_histories.push_back(obs);
			wrapper->curr_solution->target_val_histories.push_back(target_val);
		} else {
			uniform_int_distribution<int> distribution(0, wrapper->curr_solution->obs_histories.size()-1);
			int index = distribution(generator);
			wrapper->curr_solution->obs_histories[index] = obs;
			wrapper->curr_solution->target_val_histories[index] = target_val;
		}
		...
		double curr_surprise = signal - history->existing_predicted_signals[0];

		#if defined(MDEBUG) && MDEBUG
		if ((curr_sanity >= 0.0 && curr_surprise > this->best_surprise) || true) {
		#else
		if (curr_sanity >= 0.0 && curr_surprise > this->best_surprise) {
		#endif /* MDEBUG */
		...
		if (this->state_iter >= EXPERIMENT_EXPLORE_ITERS) {
			...
			#endif /* MDEBUG */
				this->state = CHASE_EXPERIMENT_STATE_TRAIN_NEW;
				this->state_iter = 0;
			} else {
