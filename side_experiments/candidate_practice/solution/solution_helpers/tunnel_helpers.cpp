...
// 			if (!scope->tunnels[t_index]->is_fail()) {
...
void update_tunnel_train_fail(SolutionWrapper* wrapper) {
	...
	// solution_tunnel->num_tries++;
	// solution_tunnel->num_train_fail++;

	// if (wrapper->potential_solution != NULL) {
	// 	Scope* potential_solution_scope = wrapper->potential_solution->scopes[wrapper->curr_tunnel_parent->id];
	// 	Tunnel* potential_solution_tunnel = potential_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	potential_solution_tunnel->num_tries++;
	// 	potential_solution_tunnel->num_train_fail++;
	// }

	// if (wrapper->best_solution != NULL) {
	// 	Scope* best_solution_scope = wrapper->best_solution->scopes[wrapper->curr_tunnel_parent->id];
	// 	Tunnel* best_solution_tunnel = best_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	best_solution_tunnel->num_tries++;
	// 	best_solution_tunnel->num_train_fail++;
	// }
...
void update_tunnel_measure_fail(SolutionWrapper* wrapper) {
	...
	// Tunnel* solution_tunnel = solution_scope->tunnels[wrapper->curr_tunnel_index];
	// solution_tunnel->num_tries++;
	// solution_tunnel->num_measure_fail++;

	// if (wrapper->potential_solution != NULL) {
	// 	Scope* potential_solution_scope = wrapper->potential_solution->scopes[wrapper->curr_tunnel_parent->id];
	// 	Tunnel* potential_solution_tunnel = potential_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	potential_solution_tunnel->num_tries++;
	// 	potential_solution_tunnel->num_measure_fail++;
	// }
	...
	// 	Tunnel* best_solution_tunnel = best_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	best_solution_tunnel->num_tries++;
	// 	best_solution_tunnel->num_measure_fail++;
	// }
...
void update_tunnel_success(SolutionWrapper* wrapper) {
	...
	// Tunnel* solution_tunnel = solution_scope->tunnels[wrapper->curr_tunnel_index];
	// solution_tunnel->num_tries++;
	// solution_tunnel->num_success++;

	// if (wrapper->potential_solution != NULL) {
	// 	Scope* potential_solution_scope = wrapper->potential_solution->scopes[wrapper->curr_tunnel_parent->id];
	// 	Tunnel* potential_solution_tunnel = potential_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	potential_solution_tunnel->num_tries++;
	// 	potential_solution_tunnel->num_success++;
	// }
	...
	// 	Tunnel* best_solution_tunnel = best_solution_scope->tunnels[wrapper->curr_tunnel_index];
	// 	best_solution_tunnel->num_tries++;
	// 	best_solution_tunnel->num_success++;
	// }
...
void measure_tunnel_vals_helper(ScopeHistory* scope_history,
	...
	if (scope->id == tunnel_parent_id) {
		sum_vals += tunnel->get_signal(scope_history->obs_history);
	} else {
		bool is_child = false;
		for (int c_index = 0; c_index < (int)scope->child_scopes.size(); c_index++) {
			if (scope->child_scopes[c_index]->id == tunnel_parent_id) {
				is_child = true;
				break;
			}
		}
