...
#else
const int TRAIN_NEW_NUM_DATAPOINTS = 125;
#endif /* MDEBUG */

const double VERIFY_RATIO = 0.2;

const int TRAIN_NUM_TRIES = 10;
...
void ExploreExperiment::train_try_helper(FocusNetwork*& best_network,
										 double& best_improvement) {
	geometric_distribution<int> num_inputs_distribution(0.2);
	int num_inputs;
	while (true) {
		num_inputs = 1 + num_inputs_distribution(generator);
		if (num_inputs <= (int)this->new_obs_histories[0].size()) {
			break;
		}
	}

	vector<int> remaining_indexes(this->new_obs_histories[0].size());
	...
	for (int i_index = 0; i_index < num_inputs; i_index++) {
		...
		input_indexes.push_back(remaining_indexes[index]);

		remaining_indexes.erase(remaining_indexes.begin() + index);
	}

	FocusNetwork* new_network = new FocusNetwork(input_indexes);
...
void ExploreExperiment::train_new_backprop(
	...
	if (history->existing_predicted_trues.size() > 0) {
		...
		if (this->state_iter >= TRAIN_NEW_NUM_DATAPOINTS) {
			...
			FocusNetwork* best_network = NULL;
			double best_improvement = 0.0;
			for (int t_index = 0; t_index < TRAIN_NUM_TRIES; t_index++) {
				train_try_helper(best_network,
								 best_improvement);
			}

			double average_instances_per_run = (double)this->sum_num_instances / (double)this->total_count;
			double global_improvement = average_instances_per_run * best_improvement;
			...
			if (is_success) {
				...
				new_eval_experiment->new_networks = vector<FocusNetwork*>{best_network};
