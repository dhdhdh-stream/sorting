...
FocusNetwork::FocusNetwork(vector<int>& input_indexes) {
	this->input_indexes = input_indexes;

	this->input = new Layer(LINEAR_LAYER);
	for (int i_index = 0; i_index < (int)input_indexes.size(); i_index++) {
...
FocusNetwork::FocusNetwork(FocusNetwork* original) {
...
FocusNetwork::FocusNetwork(ifstream& input_file) {
	string input_indexes_size_line;
	getline(input_file, input_indexes_size_line);
	int input_indexes_size = stoi(input_indexes_size_line);
	for (int i_index = 0; i_index < input_indexes_size; i_index++) {
...
void FocusNetwork::activate(vector<double>& input_vals) {
...
void FocusNetwork::backprop(double error) {
...
void FocusNetwork::save(ofstream& output_file) {
