// - having multiple outputs really bad?
...
int main(int argc, char* argv[]) {
	...
	Network* final_network = new Network(final_input_sizes, 2);
	...
	// for (int iter_index = 0; iter_index < 1000000; iter_index++) {
		...
		final_network->output->errors(0) = locs[sequence_index] - final_network->output->acti_vals(0);
		final_network->output->errors(1) = target_vals[sequence_index] - final_network->output->acti_vals(1);
		// final_network->output->errors(1) = 0.0;
	...
	for (int iter_index = 0; iter_index < 40; iter_index++) {
		...
		cout << "final_network->output->acti_vals(1): " << final_network->output->acti_vals(1) << endl;
