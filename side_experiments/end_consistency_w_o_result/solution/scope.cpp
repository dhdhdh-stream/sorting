...
void Scope::copy_from(Scope* original,
	...
	if (original->consistency_network == NULL) {
		this->consistency_network = NULL;
	} else {
		this->consistency_network = new Network(original->consistency_network);
	}

	this->existing_pre_obs = original->existing_pre_obs;
	this->existing_post_obs = original->existing_post_obs;

	this->explore_pre_obs = original->explore_pre_obs;
	this->explore_post_obs = original->explore_post_obs;
}
