...
void ForceExperiment::train_new_experiment_step(int& action,
	...
	if (state->step_index >= (int)this->best_actions.size()) {
		run->node_context = this->best_exit_next_node;
	...
	if (history->hit_branch) {
		...
		if (this->new_obs.size() >= TRAIN_NEW_NUM_SAMPLES) {
			...
			if (is_success) {
				...
				experiment->node_context = this->node_context;
				experiment->is_branch = this->is_branch;

				experiment->actions = this->best_actions;
