...
// TODO: check if backprop destroys state
// - first backprop without updating state to see how existing does
// - then learn
// - then backprop and potentially destroy state
// - then backprop without updating state
...
void no_state_update_world_model_helper(vector<vector<double>>& obs,
										vector<int>& actions,
										double target_val,
										Wrapper* wrapper) {
	WorldModel* world_model = wrapper->world_model;
