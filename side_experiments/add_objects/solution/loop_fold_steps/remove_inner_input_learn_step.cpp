...
void LoopFold::remove_inner_input_learn_outer_scope_activate_helper(
	...
	for (int i_index = 0; i_index < (int)scope_history->node_histories.size(); i_index++) {
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
				...
				remove_inner_input_learn_outer_scope_activate_helper(
					new_outer_state_vals,
...
void LoopFold::remove_inner_input_learn_inner_scope_activate_helper(
	...
	for (int i_index = 0; i_index < (int)scope_history->node_histories.size(); i_index++) {
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
				...
				ScopeNodeHistory* scope_node_history = (ScopeNodeHistory*)scope_history->node_histories[i_index][h_index];
				remove_inner_input_learn_inner_scope_activate_helper(
					new_state_vals,
					scope_node_history->inner_scope_history,
					curr_scope_context,
					curr_node_context,
...
void LoopFold::remove_inner_input_learn_activate(vector<double>& state_vals,
...
void LoopFold::remove_inner_input_learn_backprop(vector<double>& state_errors,
												 vector<bool>& states_initialized,
												 double target_val,
