...
void LoopFold::experiment_measure_outer_scope_activate_helper(
	...
	for (int i_index = 0; i_index < (int)scope_history->node_histories.size(); i_index++) {
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
				...
				experiment_measure_outer_scope_activate_helper(
					new_outer_state_vals,
...
void LoopFold::experiment_measure_inner_scope_activate_helper(
	...
	for (int i_index = 0; i_index < (int)scope_history->node_histories.size(); i_index++) {
		for (int h_index = 0; h_index < (int)scope_history->node_histories[i_index].size(); h_index++) {
			if (scope_history->node_histories[i_index][h_index]->node->type == NODE_TYPE_ACTION) {
				...
				ScopeNodeHistory* scope_node_history = (ScopeNodeHistory*)scope_history->node_histories[i_index][h_index];
				experiment_measure_inner_scope_activate_helper(
					new_state_vals,
					scope_node_history->inner_scope_history,
					curr_scope_context,
					curr_node_context,
...
void LoopFold::experiment_measure_activate(vector<double>& state_vals,
...
void LoopFold::experiment_measure_backprop(double target_val,
