...
Scope::Scope(int num_states,
			 ...
			 bool is_loop,
			 vector<bool> starting_state_network_target_indexes,
	...
	this->starting_state_network_target_indexes = starting_state_network_target_indexes;
...
Scope::Scope(ifstream& input_file) {
	...
	if (this->is_loop) {
		...
		for (int l_index = 0; l_index < starting_state_networks_size; l_index++) {
			...
			this->starting_state_network_target_indexes.push_back(stoi(target_index_line));
...
void Scope::activate(vector<double>& state_vals,
	...
	if (is_loop) {
		...
		for (int s_index = 0; s_index < (int)this->starting_state_networks.size(); s_index++) {
			if (states_initialized[this->starting_state_network_target_indexes[s_index]]) {
				if (run_helper.explore_phase == EXPLORE_PHASE_EXPERIMENT_LEARN) {
				...
				state_vals[this->starting_state_network_target_indexes[s_index]] += this->starting_state_networks[s_index]->output->acti_vals[0];
			}
		}
...
bool Scope::handle_node_activate_helper(int iter_index,
	...
	if (this->nodes[curr_node_id]->type == NODE_TYPE_ACTION) {
		...
		ActionNodeHistory* node_history = new ActionNodeHistory(action_node,
																curr_node_id);
		action_node->activate(state_vals,
							  states_initialized,
							  flat_vals,
							  predicted_score,
							  scale_factor,
			...
			if (action_node->explore_fold != NULL) {
				...
				if (matches_context) {
					...
					action_node->explore_fold->explore_score_activate(
						state_vals,	// converted to fold local state_vals in fold
						predicted_score,
						scale_factor,
						context_histories,
						run_helper,
		...
		ScopeNodeHistory* node_history = new ScopeNodeHistory(scope_node,
															  curr_node_id);
		scope_node->activate(state_vals,
							 states_initialized,
							 flat_vals,
							 predicted_score,
							 scale_factor,
		...
		FoldScoreNodeHistory* node_history = new FoldScoreNodeHistory(fold_score_node,
																	  curr_node_id);
		fold_score_node->activate(state_vals,	// converted to fold local state_vals in fold
								  predicted_score,
								  scale_factor,
								  scope_context,
								  node_context,
		...
		for (int iter_index = history->num_loop_iters-1; iter_index >= 0; iter_index--) {
			for (int h_index = (int)history->node_histories[iter_index].size()-1; h_index >= 0; h_index--) {
				...
				if (history->explore_iter_index == iter_index
					...
					backprop_explore_fold_helper(state_errors,	// converted to fold local state_errors in fold
												 target_val,
												 final_misguess,
												 predicted_score,
												 scale_factor,
			...
			for (int s_index = (int)this->starting_state_networks.size()-1; s_index >= 0; s_index--) {
				if (states_initialized[this->starting_state_network_target_indexes[s_index]]) {
					this->starting_state_networks[s_index]->backprop_errors_with_no_weight_change(
						state_errors[this->starting_state_network_target_indexes[s_index]],
						state_errors,
...
void Scope::backprop_explore_fold_helper(vector<double>& state_errors,
	...
	if (history->explore_fold_history != NULL) {
		...
		fold->explore_backprop(state_errors,	// converted to fold local state_errors in fold
							   target_val,
							   final_misguess,
							   predicted_score,
							   scale_factor,
