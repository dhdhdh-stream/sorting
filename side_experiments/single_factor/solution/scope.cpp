			...
			if (run_helper.is_explore_iter) {
				...
				if (matches_context) {
					...
					if (action_node->experiment != NULL) {
						...
						if (action_node->explore_state == EXPLORE_STATE_SETUP) {
							run_helper.phase = RUN_PHASE_EXPLORE_SETUP;

							run_helper.scale_factor_snapshot = run_helper.scale_factor;

							vector<vector<double>*> state_vals(action_node->explore_scope_context.size()+1);
							state_vals[0] = context[0].state_vals;
							for (int c_index = 0; c_index < (int)action_node->explore_scope_context.size(); c_index++) {
								state_vals[1+c_index] = context[context.size()-action_node->explore_scope_context.size()+c_index].state_vals;
							}

							run_helper.explore_starting_running_average_score = run_helper.running_average_score;
							ScoreNetworkHistory* explore_score_network_history = new ScoreNetworkHistory(action_node->explore_score_network);
							action_node->explore_score_network->activate(state_vals,
																		 explore_score_network_history);
							run_helper.explore_score_network_history = explore_score_network_history;
							run_helper.explore_score_network_output = action_node->explore_score_network->output->acti_vals[0];

							run_helper.explore_starting_running_average_misguess = run_helper.running_average_misguess;
							ScoreNetworkHistory* explore_misguess_network_history = new ScoreNetworkHistory(action_node->explore_misguess_network);
							action_node->explore_misguess_network->activate(state_vals,
																			explore_misguess_network_history);
							run_helper.explore_misguess_network_history = explore_misguess_network_history;
							run_helper.explore_misguess_network_output = action_node->explore_misguess_network->output->acti_vals[0];

							action_node->explore_iter++;
							if (action_node->explore_iter > 100000) {
								action_node->explore_state = EXPLORE_STATE_EXPLORE;
							}
						} else {
							// TODO: explore
