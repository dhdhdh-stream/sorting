...
for s_index in range(num_states):
	...
	unknown_transitions = []
	for a_index in range(NUM_ACTIONS):
		unknown_transitions.append(float(file.readline()))

	states.append([average_val, transitions, unknown_transitions])
...
unknown_node_index = global_node_index
graph.add_node(pydot.Node(global_node_index, label='unknown'))
global_node_index += 1
...
for s_index in range(num_states):
	...
	for a_index in range(NUM_ACTIONS):
		if states[s_index][2][a_index] > 0.5:
			graph.add_node(pydot.Node(global_node_index, label=pretty_print_action(a_index)))
			graph.add_edge(pydot.Edge(s_index, global_node_index))
			graph.add_edge(pydot.Edge(global_node_index, unknown_node_index))
			global_node_index += 1
		elif states[s_index][2][a_index] > 0.2:
			graph.add_node(pydot.Node(global_node_index, label=pretty_print_action(a_index)))
			graph.add_edge(pydot.Edge(s_index, global_node_index, style="dotted"))
			graph.add_edge(pydot.Edge(global_node_index, unknown_node_index, style="dotted"))
			global_node_index += 1
