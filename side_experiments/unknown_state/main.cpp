...
const int NUM_TRIES_PER_ITER = 10;
...
int main(int argc, char* argv[]) {
	...
	for (int iter_index = 0; iter_index < 20; iter_index++) {
		...
		for (int s_index = 0; s_index < NUM_TRIES_PER_ITER; s_index++) {
			WorldModel* next_model = new WorldModel(curr_model);

			uniform_int_distribution<int> state_distribution(0, (int)curr_model->states.size()-1);
			uniform_int_distribution<int> action_distribution(0, NUM_ACTIONS-1);
			next_model->add_state(state_distribution(generator),
								  action_distribution(generator));
