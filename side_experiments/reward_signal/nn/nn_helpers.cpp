...
void measure_predict_network(vector<vector<double>>& inputs,
							 vector<vector<bool>>& input_is_on,
							 vector<double>& target_vals,
							 Network* network,
							 double& average_misguess,
	...
	double sum_val_variance = 0.0;
	for (int d_index = 0; d_index < test_instances; d_index++) {
		sum_val_variance += predicted_outputs[d_index] * predicted_outputs[d_index];
	}
	val_standard_deviation = sqrt(sum_val_variance / (double)test_instances);
