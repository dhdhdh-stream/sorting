...
void BranchExperiment::measure_backprop(double target_val,
	...
	add_new_obs_data_helper(wrapper->scope_histories[0],
							this->obs_data);

	this->state_iter++;
	if (this->state_iter == EARLY_EVAL_S1_ITERS
			|| this->state_iter == EARLY_EVAL_S2_ITERS) {
		if (!compare_obs_data(this->obs_data,
							  this->hit_ratio,
							  this->state_iter,
							  wrapper)) {
			...
			if (t_score >= EARLY_SUCCESS_MIN_T_SCORE) {
				...
				#endif /* MDEBUG */
			} else if (t_score <= EARLY_FAIL_MAX_T_SCORE) {
				this->result = EXPERIMENT_RESULT_FAIL;
			}
		}
		...
		#if defined(MDEBUG) && MDEBUG
		if (rand()%2 == 0
				&& compare_obs_data(this->obs_data,
									this->hit_ratio,
									this->state_iter,
									wrapper)) {
		#else
