...
double Signal::activate_helper(int index) {
	for (int n_index = 0; n_index < (int)this->nodes.size(); n_index++) {
		this->nodes[n_index]->activate(this->input_val_histories[index][n_index],
									   this->input_is_on_histories[index][n_index]);
	}
...
void Signal::backprop_helper(int index) {
	for (int n_index = 0; n_index < (int)this->nodes.size(); n_index++) {
		this->nodes[n_index]->activate(this->input_val_histories[index][n_index],
									   this->input_is_on_histories[index][n_index]);
	}

	double target_val = this->target_val_histories[index];
	...
	for (int n_index = 0; n_index < (int)this->nodes.size(); n_index++) {
		for (int i_index = (int)this->nodes[n_index]->inputs.size()-1; i_index >= 0; i_index--) {
			...
			if (is_match) {
				...
				for (int h_index = 0; h_index < (int)this->input_val_histories.size(); h_index++) {
					this->input_val_histories[h_index][n_index].erase(
						this->input_val_histories[h_index][n_index].begin() + i_index);
				}
				for (int h_index = 0; h_index < (int)this->input_is_on_histories.size(); h_index++) {
					this->input_is_on_histories[h_index][n_index].erase(
						this->input_is_on_histories[h_index][n_index].begin() + i_index);
				}
			}
		}
	}
	...
	for (int p_index = 0; p_index < (int)this->potential_inputs.size(); p_index++) {
		for (int i_index = (int)this->potential_inputs[p_index].size()-1; i_index >= 0; i_index--) {
			...
			if (is_match) {
				...
				for (int h_index = 0; h_index < (int)this->potential_input_val_histories.size(); h_index++) {
					this->potential_input_val_histories[h_index][p_index].erase(
						this->potential_input_val_histories[h_index][p_index].begin() + i_index);
				}
				for (int h_index = 0; h_index < (int)this->potential_input_is_on_histories.size(); h_index++) {
					this->potential_input_is_on_histories[h_index][p_index].erase(
						this->potential_input_is_on_histories[h_index][p_index].begin() + i_index);
				}
			}
		}
	}
...
void Signal::copy_from(Signal* original,
	...
	this->input_val_histories = original->input_val_histories;
	this->input_is_on_histories = original->input_is_on_histories;
	...
	this->potential_input_val_histories = original->potential_input_val_histories;
	this->potential_input_is_on_histories = original->potential_input_is_on_histories;
	this->potential_count = original->potential_count;
}
