...
void Scope::save(ofstream& output_file) {
	...
	output_file << this->predict_train_last_scores.size() << endl;
	for (list<double>::iterator it = this->predict_train_last_scores.begin();
			it != this->predict_train_last_scores.end(); it++) {
		output_file << *it << endl;
	}

	output_file << this->predict_measure_last_scores.size() << endl;
	for (list<double>::iterator it = this->predict_measure_last_scores.begin();
...
void Scope::load(ifstream& input_file,
	...
	string num_predict_train_last_scores_line;
	getline(input_file, num_predict_train_last_scores_line);
	int num_predict_train_last_scores = stoi(num_predict_train_last_scores_line);
	for (int e_index = 0; e_index < num_predict_train_last_scores; e_index++) {
		string score_line;
		getline(input_file, score_line);
		this->predict_train_last_scores.push_back(stod(score_line));
	}

	string num_predict_measure_last_scores_line;
	getline(input_file, num_predict_measure_last_scores_line);
	int num_predict_measure_last_scores = stoi(num_predict_measure_last_scores_line);
	for (int e_index = 0; e_index < num_predict_measure_last_scores; e_index++) {
		string score_line;
		getline(input_file, score_line);
...
void Scope::save_for_display(ofstream& output_file) {
	...
	for (map<int, AbstractNode*>::iterator it = this->nodes.begin();
		...
		if (it->second->type != NODE_TYPE_ACTION
				|| !((ActionNode*)it->second)->is_generic) {
			output_file << it->first << endl;
			output_file << it->second->type << endl;
...
TrainScopeHistory::~TrainScopeHistory() {
	...
	for (int n_index = 0; n_index < (int)this->start_init_network_histories.size(); n_index++) {
	...
	if (this->end_score_network_history != NULL) {
		delete this->end_score_network_history;
	}
}
