...
void PredictExperiment::train_new_helper(SolutionWrapper* wrapper) {
	vector<Eigen::VectorXf> new_state_histories;
	...
	for (int h_index = 0; h_index < EXPERIMENT_TRAIN_NEW_NUM_DATAPOINTS; h_index++) {
		int sample_index = sample_distribution(generator);

		double sum_vals = 0.0;
		for (int r_index = 0; r_index < RUNS_PER_PREDICT; r_index++) {
			Eigen::VectorXf state = this->existing_state_histories[sample_index];
		...
		new_state_histories.push_back(this->existing_state_histories[sample_index]);
	...
	uniform_int_distribution<int> new_train_distribution(0, new_state_histories.size()-1);
	for (int iter_index = 0; iter_index < TRAIN_NEW_ITERS; iter_index++) {
		int rand_index = new_train_distribution(generator);

		this->new_network->activate(new_state_histories[rand_index]);
	...
	int new_count = 0;
	for (int h_index = 0; h_index < (int)new_state_histories.size(); h_index++) {
		this->existing_network->activate(new_state_histories[h_index]);
		double existing_predicted = this->existing_network->output->acti_vals[0];
		this->new_network->activate(new_state_histories[h_index]);
		double new_predicted = this->new_network->output->acti_vals[0];
	...
	double average_ratio = (existing_count + new_count)
		/ ((double)this->existing_state_histories.size()
			+ (double)new_state_histories.size());
	...
	if (local_improvement > 0.0) {
		if (this->scope_context->predict_train_last_scores.size() >= MIN_NUM_LAST_TRACK) {
			int num_better_than = 0;
			for (list<double>::iterator it = this->scope_context->predict_train_last_scores.begin();
					it != this->scope_context->predict_train_last_scores.end(); it++) {
				if (global_improvement >= *it) {
					num_better_than++;
				}
			}

			double target_better_than = LAST_BETTER_THAN_RATIO * (double)this->scope_context->predict_train_last_scores.size();

			if (num_better_than >= target_better_than) {
				is_success = true;
			}

			if (this->scope_context->predict_train_last_scores.size() >= NUM_LAST_TRACK) {
				this->scope_context->predict_train_last_scores.pop_front();
			}
			this->scope_context->predict_train_last_scores.push_back(global_improvement);
		} else {
			this->scope_context->predict_train_last_scores.push_back(global_improvement);
		}
	}
	...
	if (is_success) {
		...
		this->state = PREDICT_EXPERIMENT_STATE_MEASURE;
		this->state_iter = 0;
	} else {
		delete this;
	}
