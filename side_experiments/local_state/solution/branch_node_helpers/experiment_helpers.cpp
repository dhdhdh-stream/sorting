...
void BranchNode::experiment_step(vector<double>& obs,
	...
	if (is_branch) {
		...
		if (wrapper->run_type == RUN_TYPE_EXISTING) {
			this->branch_network->activate(wrapper->partial_states.back());
			history->score_network_history = new ScoreNetworkHistory(this->branch_network);
			this->branch_network->save(history->score_network_history);
		}
		...
		if (wrapper->run_type == RUN_TYPE_EXISTING) {
			this->original_network->activate(wrapper->partial_states.back());
			history->score_network_history = new ScoreNetworkHistory(this->original_network);
			this->original_network->save(history->score_network_history);
		}
