...
void SolutionWrapper::experiment_end(double result) {
	...
	for (int d_index = 0; d_index < DIVERSITY_RANGE; d_index++) {
		if (this->explore_experiment_histories[d_index].size() >= 2) {
			...
			for (map<ExploreExperiment*, ExploreExperimentHistory*>::iterator it = this->explore_experiment_histories[d_index].begin();
					it != this->explore_experiment_histories[d_index].end(); it++) {
	...
	for (int d_index = 0; d_index < DIVERSITY_RANGE; d_index++) {
		if (this->explore_experiment_histories[d_index].size() == 1) {
			bool is_add = false;
			for (map<ExploreExperiment*, ExploreExperimentHistory*>::iterator it = this->explore_experiment_histories[d_index].begin();
					it != this->explore_experiment_histories[d_index].end(); it++) {
	...
	for (int d_index = 0; d_index < DIVERSITY_RANGE; d_index++) {
		...
		this->explore_experiment_histories[d_index].clear();
	}
