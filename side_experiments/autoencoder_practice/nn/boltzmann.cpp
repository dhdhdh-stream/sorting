...
Boltzmann::Boltzmann() {
	...
	// this->hidden = new Layer(BOOL_LAYER);
	this->hidden = new Layer(SIGMOID_BOOL_LAYER);
...
void Boltzmann::activate(vector<double>& input_vals) {
...
void Boltzmann::backprop(vector<double>& errors) {
...
int Boltzmann::classify(vector<double>& input_vals) {
	for (int i_index = 0; i_index < (int)input_vals.size(); i_index++) {
		this->input->acti_vals[i_index] = input_vals[i_index];
	}
	...
	int result = 0;
	if (this->hidden->acti_vals[0] > 0.5) {
		result += 8;
	}
	if (this->hidden->acti_vals[1] > 0.5) {
		result += 4;
	}
	if (this->hidden->acti_vals[2] > 0.5) {
		result += 2;
	}
	if (this->hidden->acti_vals[3] > 0.5) {
		result += 1;
	}
