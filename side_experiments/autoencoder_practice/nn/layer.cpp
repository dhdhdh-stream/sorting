...
void Layer::update_structure() {
	if (this->type == BOOL_LAYER
			|| this->type == SIGMOID_BOOL_LAYER) {
		this->sum_vals = vector<double>(this->acti_vals.size());
	}
...
void Layer::activate() {
	...
	case BOOL_LAYER:
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			...
			this->sum_vals[n_index] = sum_val;

			if (sum_val > 1.0) {
				this->acti_vals[n_index] = 1.0;
			} else if (sum_val < 0.0) {
				this->acti_vals[n_index] = 0.0;
			} else {
				if (distribution(generator) > sum_val) {
					this->acti_vals[n_index] = 0.0;
				} else {
					this->acti_vals[n_index] = 1.0;
				}
	...
	case SIGMOID_BOOL_LAYER:
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			...
			double final_val = 1.0 / (1.0 + exp(-sum_val));

			this->sum_vals[n_index] = final_val;

			if (distribution(generator) > final_val) {
				this->acti_vals[n_index] = 0.0;
			} else {
				this->acti_vals[n_index] = 1.0;
			}
...
void Layer::backprop() {
	...
	case BOOL_LAYER:
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			if (this->sum_vals[n_index] < 1.0 && this->sum_vals[n_index] > 0.0) {
				for (int l_index = 0; l_index < (int)this->input_layers.size(); l_index++) {
					int layer_size = (int)this->input_layers[l_index]->acti_vals.size();
					for (int ln_index = 0; ln_index < layer_size; ln_index++) {
						this->input_layers[l_index]->errors[ln_index] +=
	...
	case SIGMOID_BOOL_LAYER:
		for (int n_index = 0; n_index < (int)this->acti_vals.size(); n_index++) {
			this->errors[n_index] *= (this->sum_vals[n_index] * (1.0 - this->sum_vals[n_index]));
