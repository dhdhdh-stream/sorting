	...
	if (!history->has_target
		...
		for (int r_index = 0; r_index < num_returns; r_index++) {
			if (absolute_distribution(generator) == 0) {
				...
				uniform_int_distribution<int> location_distribution(0, context.back().node_history.size()-1);
				AbstractNode* previous_location = (*next(context.back().node_history.begin(), location_distribution(generator))).first;
...
void BranchExperiment::explore_backprop(
	...
	if (history->has_target) {
		...
		if (select) {
			...
			this->starting_location_histories.reserve(NUM_DATAPOINTS);
			this->local_location_histories.reserve(NUM_DATAPOINTS);
			this->world_model_histories.reserve(NUM_DATAPOINTS);
			this->node_histories.reserve(NUM_DATAPOINTS);
