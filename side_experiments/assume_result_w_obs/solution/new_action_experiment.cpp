				...
				case NODE_TYPE_BRANCH:
					{
						...
						for (int i_index = (int)original_branch_node->input_types.size()-1; i_index >= 0; i_index--) {
							switch (original_branch_node->input_types[i_index]) {
							case INPUT_TYPE_GLOBAL:
							case INPUT_TYPE_LOCAL:
								new_branch_node->input_types.insert(
									new_branch_node->input_types.begin(), original_branch_node->input_types[i_index]);
								new_branch_node->input_locations.insert(
									new_branch_node->input_locations.begin(), original_branch_node->input_locations[i_index]);
								new_branch_node->input_node_context_ids.insert(new_branch_node->input_node_context_ids.begin(), -1);
								new_branch_node->input_node_contexts.insert(new_branch_node->input_node_contexts.begin(), NULL);
								new_branch_node->input_obs_indexes.insert(new_branch_node->input_obs_indexes.begin(), -1);
								break;
							case INPUT_TYPE_HISTORY:
								{
									...
									if (it == node_mappings.end()) {
										...
										new_branch_node->input_obs_indexes.insert(
											new_branch_node->input_obs_indexes.begin(), original_branch_node->input_obs_indexes[i_index]);
									}
								}
								break;
							}
