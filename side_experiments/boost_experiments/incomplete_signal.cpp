// - crazy that even when incomplete, if there's enough noise, signal still helps
...
int main(int argc, char* argv[]) {
	...
	for (int epoch_index = 0; epoch_index < 1000; epoch_index++) {
		...
		for (int i_index = 0; i_index < 1000; i_index++) {
			...
			train_signal_vals.push_back(vector<double>{val_1});
			train_true_vals.push_back(val_1 + val_2 + noise);
		}
