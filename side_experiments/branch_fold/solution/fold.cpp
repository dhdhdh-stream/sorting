...
Fold::Fold(string id,
	...
	if (this->compound_actions[0] == NULL) {
		...
		this->action_input_network = new SmallNetwork(this->curr_scope_sizes.back(),
													  this->curr_s_input_sizes.back(),
													  hidden_size,
													  this->compound_actions[0]->num_inputs);
...
void Fold::increment() {
	...
	if (this->state == STATE_INNER_SCOPE_INPUT) {
		...
		// add more time, or train specifically
