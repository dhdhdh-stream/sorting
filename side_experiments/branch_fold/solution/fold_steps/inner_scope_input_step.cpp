...
void Fold::inner_scope_input_step(vector<vector<double>>& flat_vals,
	...
	// this->action->num_inputs > 0
	this->action_input_network->activate(state_vals.back(),
										 s_input_vals.back());
	vector<double> action_input(this->action->num_inputs);
	for (int i_index = 0; i_index < this->action->num_inputs; i_index++) {
		action_input[i_index] = this->action_input_network->output->acti_vals[i_index];
	}
	...
	this->curr_fold->backprop_errors_with_no_weight_change(errors);
	...
	for (int f_index = this->sequence_length-1; f_index >= (int)this->nodes.size()+1; f_index--) {
		if (this->compound_actions[f_index] != NULL) {
			...
			this->curr_scope_input_folds[f_index]->backprop_errors_with_no_weight_change(scope_output_errors);
	...
	vector<double> action_input_errors;
	this->action->backprop(scope_input_errors[this->nodes.size()],
						   action_input_errors,
						   scope_predicted_score_errors[this->nodes.size()]);

	if (this->stage_iter <= 80000) {
		this->action_input_network->backprop_weights_with_no_error_signal(
			action_input_errors,
			0.05);
