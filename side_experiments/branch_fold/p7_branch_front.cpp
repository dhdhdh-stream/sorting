...
int main(int argc, char* argv[]) {
	...
	ifstream input_file;
	input_file.open("saves/front_network.txt");
	Network* front_network = new Network(input_file);
	input_file.close();
	...
	for (int iter_index = 1; iter_index < 1000000; iter_index++) {
		...
		vector<double> inputs;	// empty
		double predicted_score = 0.0;	// initialization doesn't matter
		scope->branch_back_activate(flat_vals,
									inputs,
									explore_path,
									explore_start_inclusive,
									explore_end_non_inclusive,
		...
		if (iter_index < 100000) {
			target_max_update = 0.1;
		} else if (iter_index < 800000) {
			target_max_update = 0.01;
		} else {
			target_max_update = 0.001;
		}
		scope->branch_back_backprop(input_errors,
									predicted_score,
									final_val,
									explore_path,
									explore_start_inclusive,
