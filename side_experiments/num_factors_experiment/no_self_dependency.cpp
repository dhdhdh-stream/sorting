/**
 * - how to create dependency on state during run:
 *   - create new state
 *   - add 2 networks:
 *     - a network updating new state
 *       - without dependency on new state
 *     - a network updating old state
 *       - with dependency on new state
 * - but old state network cannot send error signals to new state network?
 * - cannot be final network sending the error signals
 *   - old state network has to send the error signals
 *     - so error signals are constantly changing
 */
...
int main(int argc, char* argv[]) {
	...
	// Network* context_one_network = new Network(SIGMOID_LAYER, 4, 2);
	Network* context_two_network = new Network(LEAKY_LAYER, 4, 2);
	// Network* context_two_network = new Network(SIGMOID_LAYER, 2, 2);
	...
	for (int iter_index = 0; iter_index < 500000; iter_index++) {
		...
		if (iter_index % 20 == 0) {
			if (iter_index >= 100000) {
				...
				context_two_network->update(0.001);
			}
			network->update();
		}
	}
