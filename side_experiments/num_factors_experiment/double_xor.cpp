...
int main(int argc, char* argv[]) {
	...
	for (int h_index = 0; h_index < NUM_SAMPLES; h_index++) {
		...
		for (int s_index = 2; s_index < SEQUENCE_LENGTH-2; s_index++) {
			vector<double> obs;
			obs.push_back(0.0);
			obs.push_back(0.0);
			for (int i_index = 0; i_index < NUM_OBS-2; i_index++) {
	...
	Network* state_network = new Network(LEAKY_LAYER, 13, 8);
	// Network* final_network = new Network(LEAKY_LAYER, 4, 1);
	Network* final_network = new Network(LEAKY_LAYER, 8, 1);
	...
	// for (int iter_index = 0; iter_index < 800000; iter_index++) {
		...
		vector<double> state_errors(8, 0.0);
