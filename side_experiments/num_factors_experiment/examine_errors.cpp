...
int main(int argc, char* argv[]) {
	...
	// 	cout << "states:" << endl;
	// 	for (int s_index = 0; s_index < 4; s_index++) {
	// 		cout << s_index << ": " << state_one[s_index] << endl;
	// 	}
	...
	// 	cout << "errors:" << endl;
	// 	for (int i_index = 0; i_index < 4; i_index++) {
	// 		cout << i_index << ": " << network->input->errors[i_index] << endl;
	// 		network->input->errors[i_index] = 0.0;
	// 	}
	...
	vector<vector<vector<double>>> error_signals(4,
	...
	for (int h_index = 0; h_index < NUM_SAMPLES; h_index++) {
		...
		vector<double> final_errors{target_vals[h_index] - network->output->acti_vals[0]};
		network->backprop(final_errors);

		vector<double> error_one(4, 0.0);
		for (int i_index = 0; i_index < 4; i_index++) {
			error_one[i_index] += network->input->errors[i_index]
				/ (double)sequences[h_index].size();
			network->input->errors[i_index] = 0.0;
		}

		for (int a_index = 0; a_index < (int)sequences[h_index].size(); a_index++) {
			int action = sequences[h_index][a_index];
			error_signals[action][contexts[h_index][a_index]].push_back(
				error_one[action]);
		}
	}
	...
	for (int a_index = 0; a_index < 4; a_index++) {
		for (int c_index = 0; c_index < 2; c_index++) {
			double sum_error_signals = 0.0;
			for (int h_index = 0; h_index < (int)error_signals[a_index][c_index].size(); h_index++) {
				sum_error_signals += error_signals[a_index][c_index][h_index];
			}
			double error_signal_average = sum_error_signals / (double)error_signals[a_index][c_index].size();
			cout << a_index << " " << c_index << ": " << error_signal_average << endl;
		}
	}
