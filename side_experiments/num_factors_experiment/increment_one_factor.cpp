// - BTW, if context dependent, and miss, cannot easily find way to target
//   - NP-hard travelling salesman problem
	...
	for (int epoch_index = 0; epoch_index < 20; epoch_index++) {
		...
		for (int iter_index = 0; iter_index < NUM_TARGET_INCORRECT_TRIES; iter_index++) {
			...
			if (sum_distance_one != 2
				...
				if (sum_impact >= 1.0 - DIFF_THRESHOLD
						&& sum_impact <= 1.0 + DIFF_THRESHOLD) {
		...
		for (int iter_index = 0; iter_index < UPDATE_ITERS; iter_index++) {
			...
			if (is_correct_distribution(generator) == 0) {
				...
				double error = 0.0;
				if (sum_impact > 1.0 && sum_impact < 1.0 + DIFF_THRESHOLD) {
					error = 1.0 + DIFF_THRESHOLD - sum_impact;
				} else if (sum_impact < 1.0 && sum_impact > 1.0 - DIFF_THRESHOLD) {
					error = 1.0 - DIFF_THRESHOLD - sum_impact;
				}
		...
		for (int a_index = 0; a_index < 4; a_index++) {
			...
			cout << a_index << endl;
			cout << "one: " << network->output->acti_vals[0] << endl;
		}
