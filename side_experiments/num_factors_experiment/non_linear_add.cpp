...
int main(int argc, char* argv[]) {
	...
	Network* network_2 = new Network(LEAKY_LAYER, 4, 1);
	for (int iter_index = 0; iter_index < 300000; iter_index++) {
		if (iter_index % 10000 == 0) {
			cout << iter_index << endl;
		}
		...
		if (is_special) {
			vector<double> inputs{val_1, val_2, 1.0, (double)special_val};
			network_2->activate(inputs);
		} else {
			vector<double> inputs{val_1, val_2, 0.0, (double)special_val};
			network_2->activate(inputs);
		}
